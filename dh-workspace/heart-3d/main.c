...
#define window_size   (as$(usize, window_width) * window_height)
...
Vec3f calculateNormal(f32 x, f32 y, f32 r) {
...
Color calculateLighting(Vec3f normal, Vec3f viewPos, Vec3f fragPos) {
    // Normalize vectors
...
void renderHeart(RenderBuffer* buffer, f32 t) {
    ...
    // Clear z-buffer
    for (usize i = 0; i < window_size; ++i) { buffer->z_buffer[i] = -math_f32_inf; }
    ...
    for (f32 y = -0.5f; y <= 0.5f; y += render_step_size) {
        ...
        for (f32 x = -0.5f; x <= 0.5f; x += render_step_size) {
            ...
            Vec3f normal = calculateNormal(x, y, radius);
            for (f32 tz = -z; tz <= z; tz += z / render_depth_steps) {
                ...
                i32 screenX     = lroundf(
                    (rotX * perspective + 0.5f) * (window_width - 20) + 10
                );
                i32 screenY = lroundf(
                    (-rotY * perspective + 0.5f) * (window_height - 1) + 2
                );

                if (0 <= screenX && screenX < window_width && 0 <= screenY && screenY < window_height) {
                    ...
                    if (buffer->z_buffer[index] < rotZ) {
                        ...
                        Color pixelColor = calculateLighting(transformedNormal, viewPos, fragPos);
...
Err$void dh_main(int argc, const char* argv[]) {
    ...
    scope_reserveReturn(Err$void) {
        ...
        RenderBuffer buffer = {
            .canvas   = game_canvas,
            .z_buffer = (f32*)malloc(window_size * sizeof(f32))
        };
        if (!buffer.z_buffer) {
