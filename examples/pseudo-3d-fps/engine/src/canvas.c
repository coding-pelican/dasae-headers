...
Err$Ptr$engine_Canvas engine_Canvas_create(u32 width, u32 height, engine_CanvasType type) {
    ...
    let items = (Color*)malloc(len * sizeof(Color));
    if (!items) {
        free(canvas);
        return_err(mem_AllocErr_err(mem_AllocErrType_OutOfMemory));
    }
    canvas->buffer.len   = len;
    canvas->buffer.items = items;
    canvas->width        = width;
...
void engine_Canvas_destroy(engine_Canvas* canvas) {
    if (!canvas) { return; }
    if (canvas->buffer.items) {
        free(canvas->buffer.items);
    }
    free(canvas);
}
...
void engine_Canvas_clear(engine_Canvas* canvas, Color color) {
    ...
    debug_assert_nonnull(canvas->buffer.items);

    switch (canvas->type) {
    case engine_CanvasType_rgba: {
        let buffer = canvas->buffer;
        for (usize i = 0; i < buffer.len; ++i) {
            buffer.items[i] = color;
        }
    } break;
    default: {
...
void engine_Canvas_resize(engine_Canvas* canvas, u32 width, u32 height) {
    ...
    let new_items = (Color*)realloc(canvas->buffer.items, new_len * sizeof(Color));
    if (!new_items) { return; }
    canvas->buffer.len   = new_len;
    canvas->buffer.items = new_items;
    canvas->width        = width;
    canvas->height       = height;
}
...
void engine_Canvas_blit(engine_Canvas* dst, const engine_Canvas* src, i32 x, i32 y) {
    ...
    debug_assert_nonnull(dst->buffer.items);
    debug_assert_nonnull(src->buffer.items);
    ...
    if (!same_rgba_type) {
        ...
        for (i32 py = start_y; py < src_bottom; ++py) {
            for (i32 px = start_x; px < src_right; ++px) {
                ...
                const Color src_color = src->buffer.items[src_x + (src_y * src->width)];
    ...
    for (i32 py = start_y; py < src_bottom; ++py) {
        for (i32 px = start_x; px < src_right; ++px) {
            ...
            if (src_color.a == ColorChannel_max_value) {
                dst->buffer.items[px + (py * dst->width)] = src_color;
                continue;
            }

            // Alpha blending
            const Color dst_color = dst->buffer.items[px + (py * dst->width)];
            ...
            dst->buffer.items[px + (py * dst->width)] = blended;
        }
    }
}
...
void engine_Canvas_blitScaled(engine_Canvas* dst, const engine_Canvas* src, i32 x, i32 y, f32 scale) {
    ...
    for (i32 dy = start_y; dy < dst_bottom; dy++) {
        for (i32 dx = start_x; dx < dst_right; dx++) {
            ...
            switch (src->type) {
            case engine_CanvasType_rgba:
                color = src->buffer.items[src_idx];
                break;
            default:
