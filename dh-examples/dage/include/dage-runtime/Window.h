...
typedef struct dage_Window {
    ...
    dage_Input input; /* Input state snapshot (owns) */
...
typedef struct dage_Window_Cfg {
    ...
    O$dacolor_RGBA clear_color; /* Composite buffer clear color */
...
$attr($inline_always)
$static fn_((dage_Window_isMousePressed(const dage_Window* self, dage_MouseBtn btn))(bool));
/// @brief Check if mouse is currently held
$attr($inline_always)
$static fn_((dage_Window_isMouseHeld(const dage_Window* self, dage_MouseBtn btn))(bool));
/// @brief Check if mouse was just released
$attr($inline_always)
$static fn_((dage_Window_isMouseReleased(const dage_Window* self, dage_MouseBtn btn))(bool));
...
fn_((dage_Window_isKeyPressed(const dage_Window* self, dage_KeyCode key))(bool)) {
    claim_assert_nonnull(self);
    return dage_Input_isKeyPressed(&self->input, key);
};

fn_((dage_Window_isKeyHeld(const dage_Window* self, dage_KeyCode key))(bool)) {
    claim_assert_nonnull(self);
    return dage_Input_isKeyHeld(&self->input, key);
};

fn_((dage_Window_isKeyReleased(const dage_Window* self, dage_KeyCode key))(bool)) {
    claim_assert_nonnull(self);
    return dage_Input_isKeyReleased(&self->input, key);
};

fn_((dage_Window_isMousePressed(const dage_Window* self, dage_MouseBtn btn))(bool)) {
    claim_assert_nonnull(self);
    return dage_Input_isMousePressed(&self->input, btn);
};

fn_((dage_Window_isMouseHeld(const dage_Window* self, dage_MouseBtn btn))(bool)) {
    claim_assert_nonnull(self);
    return dage_Input_isMouseHeld(&self->input, btn);
};

fn_((dage_Window_isMouseReleased(const dage_Window* self, dage_MouseBtn btn))(bool)) {
    claim_assert_nonnull(self);
    return dage_Input_isMouseReleased(&self->input, btn);
};

fn_((dage_Window_getMousePos(const dage_Window* self))(m_V2i32)) {
    claim_assert_nonnull(self);
    return dage_Input_getMousePos(&self->input);
};
...
fn_((dage_Window_getPixels(const dage_Window* self))(S_const$u32)) {
    ...
    /* Canvas grid items are dacolor_RGBA which is u32-compatible */
...
#endif /* defined(__cplusplus) */
#endif /* dage_runtime_Window__included */
