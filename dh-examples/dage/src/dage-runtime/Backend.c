#include "dage-runtime/Backend.h"
...
fn_((dage_Backend_pumpEventsAlloc(dage_Backend self, mem_Alctr gpa, usize capacity_hint))(E$dage_Backend_PumpedEvents) $scope) {
    self = dage_Backend_ensureValid(self);
    let cap = capacity_hint == 0 ? 128 : capacity_hint;
    /* Allocate buffer - error boundary */
    let capped = u_castS$((S$dage_RawEvent)(try_(mem_Alctr_alloc($trace gpa, typeInfo$(dage_RawEvent), cap))));
...
fn_((dage_Backend_PumpedEvents_free(dage_Backend_PumpedEvents* self, mem_Alctr gpa))(void)) {
    claim_assert_nonnull(self);
    mem_Alctr_free($trace gpa, u_anyS(self->items));
    asg_l((self)(cleared()));
};
