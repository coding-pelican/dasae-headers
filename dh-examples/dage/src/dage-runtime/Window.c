...
T_use_E$($set(mem_E)(P$dage_Viewport));
T_use$((dage_Viewport)(
    ArrList_fixed,
    ArrList_init,
    ArrList_fini,
...
fn_((dage_Window_init(dage_Window_Cfg cfg))(E$dage_Window) $guard) {
    ...
    window.cfg.clear_color = orelse_((cfg.clear_color)(dacolor_RGBA_fromOpaque(0x18, 0x18, 0x18)));
    ...
    dage_Input_init(&window.input);

    /* Create composite buffer */
    let composite_buf = u_castP$((P$dage_Canvas)(try_(mem_Alctr_create($trace gpa, typeInfo$(dage_Canvas)))));
    errdefer_($ignore, mem_Alctr_destroy($trace gpa, u_anyP(composite_buf)));
...
fn_((dage_Window_fini(dage_Window* self))(void)) {
    ...
    mem_Alctr_destroy($trace self->gpa, u_anyP(self->composite_buf));
...
fn_((dage_Window_getMouseCanvasPos(const dage_Window* self, dage_ViewportId viewport_id))(O$m_V2i32) $scope) {
    ...
    let mouse_pos = dage_Input_getMousePos(&self->input);
...
fn_((dage_Window_composite(dage_Window* self))(void)) {
    ...
    dage_Canvas_clear(self->composite_buf, some$((O$dacolor_RGBA)(self->cfg.clear_color)));
...
fn_((dage_Window_endFrame(dage_Window* self))(void)) {
    claim_assert_nonnull(self);
    dage_Input_endFrame(&self->input);
};
...
fn_((dage_Window_handleResize(dage_Window* self, m_V2u32 new_size))(E$void) $scope) {
    ...
    /* Clamp to limits */
    let clamped_w = pri_clamp(new_size.x, self->cfg.min_size.x, self->cfg.max_size.x);
    let clamped_h = pri_clamp(new_size.y, self->cfg.min_size.y, self->cfg.max_size.y);
