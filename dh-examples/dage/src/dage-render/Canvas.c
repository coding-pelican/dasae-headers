...
fn_((dage_Canvas_init(dage_Canvas_Cfg cfg))(E$dage_Canvas) $guard) {
    ...
    let grid_buf = u_castS$((S$dacolor_RGBA)(try_(mem_Alctr_alloc($trace gpa, typeInfo$(dacolor_RGBA), area))));
    errdefer_($ignore, mem_Alctr_free($trace gpa, u_anyS(grid_buf)));
    canvas.gird = Grid_fromS$(Grid$dacolor_RGBA, grid_buf, cfg.width, cfg.height);

    canvas.default_color = orelse_((cfg.default_color)(dacolor_RGBA_blank));
    dage_Canvas_clear(&canvas, none$((O$dacolor_RGBA)));

    return_ok(canvas);
} $unguarded(fn);

fn_((dage_Canvas_fini(dage_Canvas* self, mem_Alctr gpa))(void)) {
    claim_assert_nonnull(self);
    gpa = mem_Alctr_ensureValid(gpa);
    mem_Alctr_free($trace gpa, u_anyS(self->gird.items));
};

fn_((dage_Canvas_resize(dage_Canvas* self, mem_Alctr gpa, u32 width, u32 height))(E$void) $scope) {
    claim_assert_nonnull(self);
    ...
    let new_items = u_castS$((S$dacolor_RGBA)(try_(mem_Alctr_realloc($trace gpa, u_anyS(self->gird.items), new_len))));
    self->gird = Grid_fromS$(Grid$dacolor_RGBA, new_items, width, height);
    log_debug("canvas resized: %u x %u -> %u x %u", Grid_width(self->gird), Grid_height(self->gird), width, height);
    return_ok({});
} $unscoped(fn);

fn_((dage_Canvas_clear(dage_Canvas* self, O$dacolor_RGBA other_color))(void)) {
    claim_assert_nonnull(self);
    claim_assert_nonnullS(self->gird.items);
...
void dage_Canvas_drawPixel(dage_Canvas* self, i32 x, i32 y, dacolor_RGBA color) {
    ...
    *Grid_at(self->gird, x, y) = dacolor_RGBA_blendAlpha(color, dst);
}

// Helper function to draw a line using Bresenham's algorithm
void dage_Canvas_drawLine(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, dacolor_RGBA color) {
...
void dage_Canvas_drawHLine(dage_Canvas* self, i32 x1, i32 x2, i32 y, dacolor_RGBA color) {
    claim_assert_nonnull(self);
    claim_assert_nonnull(self->gird.items.ptr);

    // Ensure x1 is the leftmost point
    if (x2 < x1) { pri_swap(&x1, &x2); }

    // Clip to self bounds
    if (y < 0 || as$(i32)(Grid_height(self->gird)) <= y) { return; }
    x1 = pri_max(0, x1);
    x2 = pri_min(as$(i32)(Grid_width(self->gird)) - 1, x2);
...
void dage_Canvas_drawVLine(dage_Canvas* self, i32 x, i32 y1, i32 y2, dacolor_RGBA color) {
    claim_assert_nonnull(self);
    claim_assert_nonnull(self->gird.items.ptr);

    // Ensure y1 is the topmost point
    if (y2 < y1) { pri_swap(&y1, &y2); }

    // Clip to self bounds
    if (x < 0 || as$(i32)(Grid_width(self->gird)) <= x) { return; }
    y1 = pri_max(0, y1);
    y2 = pri_min(as$(i32)(Grid_height(self->gird)) - 1, y2);
...
void dage_Canvas_drawRect(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, dacolor_RGBA color) {
...
$inline_always void plotCirclePoints(dage_Canvas* self, i32 cx, i32 cy, i32 x, i32 y, dacolor_RGBA color) {
...
void dage_Canvas_drawCircle(dage_Canvas* self, i32 cx, i32 cy, i32 radius, dacolor_RGBA color) {
...
void dage_Canvas_drawRing(dage_Canvas* self, i32 cx, i32 cy, i32 r_inner, i32 r_outer, dacolor_RGBA color) {
...
$inline_always
    f32
    degToRad(f32 deg) { return (deg * math_f32_pi) / 180.0f; }
...
$inline_always void plotArcPoints(dage_Canvas* self, i32 cx, i32 cy, i32 x, i32 y, f32 start_rad, f32 end_rad, dacolor_RGBA color) {
...
void dage_Canvas_drawArc(dage_Canvas* self, i32 cx, i32 cy, i32 radius, f32 start_angle, f32 end_angle, dacolor_RGBA color) {
    // Convert degrees to radians
...
void dage_Canvas_drawAngleRing(dage_Canvas* self, i32 cx, i32 cy, i32 r_inner, i32 r_outer, f32 start_angle, f32 end_angle, dacolor_RGBA color) {
...
void dage_Canvas_drawCapsule(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, dacolor_RGBA color) {
    if (x2 < x1) { pri_swap(&x1, &x2); }
    if (y2 < y1) { pri_swap(&y1, &y2); }
...
$inline_always
    i32
    iround(f32 x) { return (i32)(x + 0.5f); }

// Draw a thick line by drawing multiple offset lines
void dage_Canvas_drawLineThick(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, f32 thickness, dacolor_RGBA color) {
    if (thickness <= 1.0f) {
...
void dage_Canvas_drawRectThick(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, f32 thickness, dacolor_RGBA color) {
    if (x2 < x1) { pri_swap(&x1, &x2); }
    if (y2 < y1) { pri_swap(&y1, &y2); }
...
void dage_Canvas_drawRectBorderByCutout(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, i32 thickness, dacolor_RGBA inner_color, dacolor_RGBA border_color) {
    if (x2 < x1) { pri_swap(&x1, &x2); }
    if (y2 < y1) { pri_swap(&y1, &y2); }
...
void dage_Canvas_fillRect(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, dacolor_RGBA color) {
    // Ensure x1 < x2 and y1 < y2
    if (x2 < x1) { pri_swap(&x1, &x2); }
    if (y2 < y1) { pri_swap(&y1, &y2); }
...
void dage_Canvas_fillCircle(dage_Canvas* self, i32 cx, i32 cy, i32 radius, dacolor_RGBA color) {
    // if (radius <= 0) { return; }                                                // Zero or Negative radius is invalid
    // if (radius == 1) { return dage_Canvas_drawPixel(self, cx, cy, color); } // Fall-back to a single pixel
...
void dage_Canvas_fillRingByCutout(dage_Canvas* self, i32 cx, i32 cy, i32 r_inner, i32 r_outer, dacolor_RGBA inner_color, dacolor_RGBA outer_color) {
...
void dage_Canvas_fillRingByScanlines(dage_Canvas* self, i32 cx, i32 cy, i32 r_inner, i32 r_outer, dacolor_RGBA color) {
...
$inline_always void drawHLineAngleClipped(dage_Canvas* self, i32 x1, i32 x2, i32 y, i32 cx, i32 cy, f32 start_rad, f32 end_rad, dacolor_RGBA color) {
    if (x2 < x1) { pri_swap(&x1, &x2); }
...
$inline_always void drawVLineAngleClipped(dage_Canvas* self, i32 y1, i32 y2, i32 x, i32 cx, i32 cy, f32 start_rad, f32 end_rad, dacolor_RGBA color) {
    if (y2 < y1) { pri_swap(&y1, &y2); }
...
void dage_Canvas_fillArc(dage_Canvas* self, i32 cx, i32 cy, i32 radius, f32 start_angle, f32 end_angle, dacolor_RGBA color) {
    // Convert to radians
    f32 start_rad = degToRad(start_angle);
    f32 end_rad = degToRad(end_angle);
...
void dage_Canvas_fillAngleRingByCutout(dage_Canvas* self, i32 cx, i32 cy, i32 r_inner, i32 r_outer, f32 start_angle, f32 end_angle, dacolor_RGBA inner_color, dacolor_RGBA outer_color) {
    // 1) Fill outer
...
void dage_Canvas_fillCapsule(dage_Canvas* self, i32 x1, i32 y1, i32 x2, i32 y2, dacolor_RGBA color) {
    if (x2 < x1) { pri_swap(&x1, &x2); }
    if (y2 < y1) { pri_swap(&y1, &y2); }
...
void dage_Canvas_blit(dage_Canvas* dst, const dage_Canvas* src, i32 x, i32 y) {
    ...
    const i32 src_right = pri_min(x + as$(i32)(Grid_width(src->gird)), as$(i32)(Grid_width(dst->gird)));
    const i32 src_bottom = pri_min(y + as$(i32)(Grid_height(src->gird)), as$(i32)(Grid_height(dst->gird)));
    const i32 start_x = pri_max(0, x);
    const i32 start_y = pri_max(0, y);
    ...
    for (i32 py = start_y; py < src_bottom; ++py) {
        for (i32 px = start_x; px < src_right; ++px) {
            ...
            const dacolor_RGBA src_color = src->gird.items.ptr[as$(usize)(src_x) + (as$(usize)(src_y)*Grid_width(src->gird))];
            const dacolor_RGBA dst_color = dst->gird.items.ptr[as$(usize)(px) + (as$(usize)(py)*Grid_width(dst->gird))];

            // Alpha blending
            dst->gird.items.ptr[as$(usize)(px) + (as$(usize)(py)*Grid_width(dst->gird))] = dacolor_RGBA_blendAlpha(src_color, dst_color);
        }
    }
    ...
    if (!same_rgba_type) {
        ...
        for (i32 py = start_y; py < src_bottom; ++py) {
            for (i32 px = start_x; px < src_right; ++px) {
                const i32   src_x     = px - x;
                const i32   src_y     = py - y;
                const dacolor_RGBA src_color = src->buffer.items.ptr[src_x + (src_y * Grid_width(src->buffer))];
...
void dage_Canvas_blitScaled(dage_Canvas* dst, const dage_Canvas* src, i32 x, i32 y, f32 scale) {
    ...
    const i32 dst_right = pri_min(x + scaled_width, as$(i32)(Grid_width(dst->gird)));
    const i32 dst_bottom = pri_min(y + scaled_height, as$(i32)(Grid_height(dst->gird)));
    const i32 start_x = pri_max(0, x);
    const i32 start_y = pri_max(0, y);
