#include "dage-context/Runtime.h"
#include <dh/ArrList.h>

T_use_E$($set(mem_E)(P$P$dage_Window));
T_use$((P$dage_Window)(
    ArrList_init,
    ArrList_fini,
...
fn_((dage_Runtime_fini(dage_Runtime* self))(void)) {
    ...
    for_(($s(self->windows.items))(win_ptr)) {
        ...
        mem_Alctr_destroy($trace self->gpa, u_anyP(win));
    } $end(for);
    /* Free window list */
...
fn_((dage_Runtime_createWindow(dage_Runtime* self, dage_Runtime_WindowCfg cfg))(E$dage_WindowId) $guard) {
    ...
    let win = u_castP$((P$dage_Window)(try_(mem_Alctr_create($trace self->gpa, typeInfo$(dage_Window)))));
    errdefer_($ignore, mem_Alctr_destroy($trace self->gpa, u_anyP(win)));
...
fn_((dage_Runtime_processEvents(dage_Runtime* self))(void)) {
    ...
    for_(($s(events))(raw)) {
        if_some((dage_Runtime_findWindowByTarget(self, raw->target_id))(win)) {
            ...
            dage_Input_applyEvent(&win->input, &raw->base);
