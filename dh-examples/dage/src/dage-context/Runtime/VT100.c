...
$static fn_((VT100_createTarget(
    ...
    let raw = try_(mem_Alctr_alloc(self->gpa, typeInfo$(char), buf_size));
...
fn_((engine_core_VT100_init(engine_core_VT100_Cfg cfg))(E$engine_core_VT100) $guard) {
    ...
    let raw = try_(mem_Alctr_alloc(gpa, typeInfo$(engine_core_VT100), 1));
    let self = (engine_core_VT100*)raw.ptr;
    errdefer_(mem_Alctr_free(gpa, raw));
