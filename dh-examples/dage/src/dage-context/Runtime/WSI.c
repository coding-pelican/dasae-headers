#include "dage-context/Runtime/WSI.h"

/*========== Internal Forward Declarations ==================================*/

$static fn_((WSI__findTarget(dage_Runtime_WSI* self, dage_TargetId id))(O$P$dage_Runtime_WSI_Target));
$static fn_((WSI__findFreeSlot(dage_Runtime_WSI* self))(O$P$dage_Runtime_WSI_Target));
$static fn_((WSI__pushEvent(dage_Runtime_WSI_Target* target, dage_Event event))(void));

/* Unsupported platform fallbacks */
pp_if_(pp_true)(pp_then_(
    $attr($inline_always $must_check $maybe_unused)
    $static fn_((WSI__unsupported_platformInit(P$dage_Runtime_WSI self))(E$void));
    $attr($inline_always $maybe_unused)
    $static fn_((WSI__unsupported_platformFini(P$dage_Runtime_WSI self))(void));
    $attr($inline_always $must_check $maybe_unused)
    $static fn_((WSI__unsupported_createTarget(P$dage_Runtime_WSI self, dage_Target_Cfg cfg))(E$dage_TargetId));
    $attr($inline_always $maybe_unused)
    $static fn_((WSI__unsupported_destroyTarget(P$dage_Runtime_WSI self, dage_TargetId id))(void));
    $attr($inline_always $maybe_unused)
    $static fn_((WSI__unsupported_pumpEvents(P$dage_Runtime_WSI self, S$dage_RawEvent out))(S$dage_RawEvent));
    $attr($inline_always $maybe_unused)
    $static fn_((WSI__unsupported_present(P$dage_Runtime_WSI self, dage_TargetId id, S_const$u32 pixels, u32 w, u32 h))(void));
    $attr($inline_always $maybe_unused)
    $static fn_((WSI__unsupported_getTargetMetrics(P$dage_Runtime_WSI self, dage_TargetId id))(dage_TargetMetrics));
));
/* Windows declarations */
pp_if_(plat_is_windows)(pp_then_(
    $attr($inline_always $must_check)
    $static fn_((WSI__windows_platformInit(P$dage_Runtime_WSI self))(E$void));
    $attr($inline_always)
    $static fn_((WSI__windows_platformFini(P$dage_Runtime_WSI self))(void));
    $attr($inline_always $must_check)
    $static fn_((WSI__windows_createTarget(P$dage_Runtime_WSI self, dage_Target_Cfg cfg))(E$dage_TargetId));
    $attr($inline_always)
    $static fn_((WSI__windows_destroyTarget(P$dage_Runtime_WSI self, dage_TargetId id))(void));
    $attr($inline_always)
    $static fn_((WSI__windows_pumpEvents(P$dage_Runtime_WSI self, S$dage_RawEvent out))(S$dage_RawEvent));
    $attr($inline_always)
    $static fn_((WSI__windows_present(P$dage_Runtime_WSI self, dage_TargetId id, S_const$u32 pixels, u32 w, u32 h))(void));
    $attr($inline_always)
    $static fn_((WSI__windows_getTargetMetrics(P$dage_Runtime_WSI self, dage_TargetId id))(dage_TargetMetrics));
));
/* Linux/X11 declarations */
pp_if_(dage_Runtime_WSI__use_x11)(pp_then_(
    $attr($inline_always $must_check)
    $static fn_((WSI__x11_platformInit(P$dage_Runtime_WSI self))(E$void));
    $attr($inline_always)
    $static fn_((WSI__x11_platformFini(P$dage_Runtime_WSI self))(void));
    $attr($inline_always $must_check)
    $static fn_((WSI__x11_createTarget(P$dage_Runtime_WSI self, dage_Target_Cfg cfg))(E$dage_TargetId));
    $attr($inline_always)
    $static fn_((WSI__x11_destroyTarget(P$dage_Runtime_WSI self, dage_TargetId id))(void));
    $attr($inline_always)
    $static fn_((WSI__x11_pumpEvents(P$dage_Runtime_WSI self, S$dage_RawEvent out))(S$dage_RawEvent));
    $attr($inline_always)
    $static fn_((WSI__x11_present(P$dage_Runtime_WSI self, dage_TargetId id, S_const$u32 pixels, u32 w, u32 h))(void));
    $attr($inline_always)
    $static fn_((WSI__x11_getTargetMetrics(P$dage_Runtime_WSI self, dage_TargetId id))(dage_TargetMetrics));
));
...
$static let WSI__platformInit = pp_if_(plat_is_windows)(
    ...
    pp_else_(pp_if_(dage_Runtime_WSI__use_x11)(
...
fn_((dage_Runtime_WSI_init(dage_Runtime_WSI_Cfg cfg))(E$P$dage_Runtime_WSI) $guard) {
    let gpa = mem_Alctr_ensureValid(cfg.gpa);

    /* Allocate WSI instance via gpa (not malloc) */
    let wsi = u_castP$((P$dage_Runtime_WSI)(try_(mem_Alctr_create($trace gpa, typeInfo$(dage_Runtime_WSI)))));
    errdefer_($ignore, mem_Alctr_destroy($trace gpa, u_anyP(wsi)));

    /* Initialize fields */
    wsi->gpa = gpa;
    mem_set0Bytes(mem_asBytesMut(u_anyP(&wsi->targets)));
    wsi->target_count = 1;
    mem_set0Bytes(mem_asBytesMut(u_anyP(&wsi->display)));
    mem_set0Bytes(mem_asBytesMut(u_anyP(&wsi->impl)));
...
fn_((dage_Runtime_WSI_fini(dage_Runtime_WSI** self))(void)) {
    claim_assert_nonnull(self);

    let wsi = *self;
    if (!wsi->initialized) {
        mem_Alctr_destroy($trace wsi->gpa, u_anyP(wsi));
    ...
    let gpa = wsi->gpa;
    mem_Alctr_destroy($trace gpa, u_anyP(wsi));
    mem_set0Bytes(mem_asBytesMut(u_anyP(self)));
};

fn_((dage_Runtime_WSI_backend(dage_Runtime_WSI* self))(dage_Backend)) {
    claim_assert_nonnull(self);
...
fn_((dage_Runtime_WSI_getNativeHandle(dage_Runtime_WSI* self, dage_TargetId id))(dage_Runtime_WSI_Handle)) {
    claim_assert_nonnull(self);
    return ensureNonnull(unwrap_(WSI__findTarget(self, id))->handle);
};

fn_((dage_Runtime_WSI_getNativeDisplay(dage_Runtime_WSI* self))(dage_Runtime_WSI_Display)) {
    claim_assert_nonnull(self);
...
fn_((WSI__findTarget(dage_Runtime_WSI* self, dage_TargetId id))(O$P$dage_Runtime_WSI_Target) $scope) {
...
fn_((WSI__findFreeSlot(dage_Runtime_WSI* self))(O$P$dage_Runtime_WSI_Target) $scope) {
    for_(($s(A_ref(self->targets)))(target)) {
        if (!target->active) {
            return_some(target);
        }
...
fn_((WSI__pushEvent(dage_Runtime_WSI_Target* target, dage_Event event))(void)) {
    if (target->pending_event_count < dage_Runtime_WSI_max_pending_events) {
...
fn_((WSI__unsupported_pumpEvents(P$dage_Runtime_WSI self, S$dage_RawEvent out))(S$dage_RawEvent)) {
    let_ignore = self;
    return S_slice((out)$r(0, 0));
};

fn_((WSI__unsupported_present(P$dage_Runtime_WSI self, dage_TargetId id, S_const$u32 pixels, u32 w, u32 h))(void)) {
    let_ignore = self;
    let_ignore = id;
    let_ignore = pixels;
    let_ignore = w;
...
fn_((WSI__unsupported_getTargetMetrics(P$dage_Runtime_WSI self, dage_TargetId id))(dage_TargetMetrics)) {
    let_ignore = self;
    let_ignore = id;
...
$static fn_((WSI__windows_findByHWND(P$dage_Runtime_WSI self, HWND hwnd))(O$P$dage_Runtime_WSI_Target) $scope) {
    claim_assert_nonnull(self);
    for_(($s(A_ref(self->targets)))(target)) {
...
#if UNUSED_CODE
    if_none(some$(O$P$dage_Runtime_WSI, self)) {
        return DefWindowProcW(hwnd, msg, wParam, lParam);
    }
#endif /* UNUSED_CODE */
...
fn_((WSI__windows_platformFini(P$dage_Runtime_WSI self))(void)) {
...
fn_((WSI__windows_createTarget(P$dage_Runtime_WSI self, dage_Target_Cfg cfg))(E$dage_TargetId) $scope) {
    ...
    if (cfg.title.len > 0) {
        let limit_len = pri_min(cfg.title.len, A_len(title_wide) - 1);
    ...
    mem_set0Bytes(mem_asBytesMut(u_anyP(target)));
    target->id = id;
    target->active = true;
    target->handle = hwnd;
...
fn_((WSI__windows_destroyTarget(P$dage_Runtime_WSI self, dage_TargetId id))(void)) {
...
fn_((WSI__windows_pumpEvents(P$dage_Runtime_WSI self, S$dage_RawEvent out))(S$dage_RawEvent)) {
    MSG msg = cleared();
...
fn_((WSI__windows_present(P$dage_Runtime_WSI self, dage_TargetId id, S_const$u32 pixels, u32 w, u32 h))(void)) {
    let target = orelse_((WSI__findTarget(self, id))(return));
...
fn_((WSI__windows_getTargetMetrics(P$dage_Runtime_WSI self, dage_TargetId id))(dage_TargetMetrics) $scope) {
...
$static fn_((WSI__x11_findByWindow(P$dage_Runtime_WSI self, u64 window))(O$P$dage_Runtime_WSI_Target)) {
    for (usize i = 0; i < dage_Runtime_WSI_max_targets; ++i) {
        let target = A_at(self->targets[i]);
        if (target->active && target->handle == window) {
            return some$(O$P$dage_Runtime_WSI_Target, target);
        }
    }
    return none$(O$P$dage_Runtime_WSI_Target);
}
...
fn_((WSI__x11_platformFini(P$dage_Runtime_WSI self))(void)) {
...
fn_((WSI__x11_createTarget(P$dage_Runtime_WSI self, dage_Target_Cfg cfg))(E$dage_TargetId) $guard) {
    ...
    let alloc_result = mem_Alctr_alloc(self->gpa, typeInfo$(u32), buffer_len);
    if_err(alloc_result) {
        XFreeGC(dpy, gc);
    ...
    *target = (dage_Runtime_WSI_Target){
        .id = id,
        .active = true,
        .handle = (u64)window,
        .device = (u64)gc,
...
fn_((WSI__x11_destroyTarget(P$dage_Runtime_WSI self, dage_TargetId id))(void)) {
    ...
    if (target->image.pixels.ptr != cleared()) {
        ...
        mem_Alctr_free(self->gpa, u_anyS(target->image.pixels));
    }
    if (target->device != 0) {
...
fn_((WSI__x11_pumpEvents(P$dage_Runtime_WSI self, S$dage_RawEvent out))(S$dage_RawEvent)) {
    Display* dpy = (Display*)self->display;
    usize count = 0;
...
fn_((WSI__x11_present(P$dage_Runtime_WSI self, dage_TargetId id, S_const$u32 pixels, u32 w, u32 h))(void)) {
    let target_opt = WSI__findTarget(self, id);
    if_none(target_opt) { return; }
    let target = unwrap_(target_opt);
    ...
    usize pixel_count = pri_min((usize)(w * h), target->image.pixels.len);
...
fn_((WSI__x11_getTargetMetrics(P$dage_Runtime_WSI self, dage_TargetId id))(dage_TargetMetrics)) {
    let target_opt = WSI__findTarget(self, id);
    if_none(target_opt) {
        return (dage_TargetMetrics){ .should_close = true };
    }
...
#endif /* dage_Runtime_WSI__use_x11 */
