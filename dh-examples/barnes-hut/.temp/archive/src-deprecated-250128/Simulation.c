...
Err$void Simulation_collide(Simulation* self) {
    scope_reserveReturn(Err$void) {
        ...
        for (usize current = 0; current < indices.len - 1; ++current) {
...
void Simulation_resolve(Simulation* self, usize lhs, usize rhs) {
    ...
    if (0.0f <= d_dot_v && m_V2f32_ne(d, m_V2f32_zero)) {
        ...
        b1->pos = m_V2f32_sub(b1->pos, m_V2f32_scale(tmp, weight1));
        b2->pos = m_V2f32_add(b2->pos, m_V2f32_scale(tmp, weight2));
        return;
    }
    ...
    b1->pos = m_V2f32_sub(b1->pos, m_V2f32_scale(v1, t));
    b2->pos = m_V2f32_sub(b2->pos, m_V2f32_scale(v2, t));
    ...
    b1->vel = v1_new;
    b2->vel = v2_new;
    b1->pos = m_V2f32_add(b1->pos, m_V2f32_scale(v1_new, t));
    b2->pos = m_V2f32_add(b2->pos, m_V2f32_scale(v2_new, t));
}
