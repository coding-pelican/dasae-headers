...
static BOOL calculate_cell_size_dwrite(TerminalContext* ctx) {
    ...
    double fontHeightInPixels = points_to_pixels(effectiveSize, dpiY);
    ...
    // Calculate using proper DPI scaling
    float designUnitsPerEm = (float)metrics.designUnitsPerEm;
    float scaleFactor = fontHeightInPixels / designUnitsPerEm;

    // Cell height with line height multiplier
    float baseHeight = (metrics.ascent + metrics.descent) * scaleFactor;
    ctx->cellHeight = baseHeight * ctx->settings.lineHeight;

    // Cell width from glyph advance width
    UINT16 glyphIndex;
    UINT32 codePoint = 'i';
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ctx->cellWidth = glyphMetrics.advanceWidth * scaleFactor;

            // Windows Terminal applies pixel snapping - round to nearest pixel
            ctx->cellWidth = round(ctx->cellWidth);
            ctx->cellHeight = round(ctx->cellHeight);
        }
    }

    printf("[+] Cell size (DirectWrite): %.2f x %.2f pixels\n", ctx->cellWidth, ctx->cellHeight);
    printf("    DPI: %dx%d, Font: %dpt = %.2fpx, Line height: %.2f\n", dpiX, dpiY, effectiveSize, fontHeightInPixels, ctx->settings.lineHeight);
...
static void calculate_cell_size(TerminalContext* ctx) {
    ...
    if (calculate_cell_size_dwrite(ctx)) {
        // Verify against empirical measurements
        // If the values seem wrong, apply corrections
        if (ctx->cellWidth < 5.0 || ctx->cellWidth > 15.0) {
            printf("[!] Cell width %.2f seems incorrect, applying correction\n", ctx->cellWidth);
        }
        return;
    }

    // Fallback to GDI with proper DPI scaling
    ...
    double fontHeightInPixels = points_to_pixels(effectiveSize, logPixelsY);
    int fontHeight = -(int)round(fontHeightInPixels);
    ...
    if (font) {
        ...
        ctx->cellWidth = (double)tm.tmAveCharWidth;
        ctx->cellHeight = (double)tm.tmHeight * ctx->settings.lineHeight;

        printf("[+] Cell size (GDI): %.2f x %.2f pixels\n", ctx->cellWidth, ctx->cellHeight);
...
static TerminalPosition screen_to_terminal_pos(POINT screenPt, TerminalContext* ctx) {
    ...
    if (ctx->cellWidth > 0 && ctx->cellHeight > 0) {
        pos.charX = (int)floor((textX + ctx->cellWidth * 0.5) / ctx->cellWidth);
        pos.charY = (int)floor((textY + ctx->cellHeight * 0.5) / ctx->cellHeight);
    }
...
int main(void) {
    printf("=== Windows Terminal Monitor v4 ===\n\n");
