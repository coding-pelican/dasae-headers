...
fn_scope_ext(engine_Input_init(mem_Allocator allocator), Err$Ptr$engine_Input) {
    ...
    with_(let mouse = input->mouse){
        ...
        mouse->cursor.curr_pos = math_Vec2i_zero;
        mouse->cursor.prev_pos = math_Vec2i_zero;

        mouse->wheel.curr_scroll_amount = math_Vec2f_zero;
        mouse->wheel.prev_scroll_amount = math_Vec2f_zero;
    }
...
fn_scope(engine_Input_update(engine_Input* self), Err$void) {
    $unused(self);
...
fn_scope(engine_InputEventBuffer_pop(engine_Input* self), Opt$engine_InputEvent) {
    $unused(self);
    return_none();
    /* TODO: Implement this function */
} unscoped;
