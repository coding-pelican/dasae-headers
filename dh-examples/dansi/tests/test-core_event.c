...
TEST_fn_("dansi-core/Event: parse focus-in and focus-out CSI" $scope) {
    ...
    pattern_((dansi_Event_focus)(focus)) try_(TEST_expect(focus == dansi_Event_Focus_in)) $end(pattern);
    default_() try_(TEST_expect(false)) $end(default);
    } $end(match);
    $suppress_(switch_enum)(match_(focus_out)) {
    pattern_((dansi_Event_focus)(focus)) try_(TEST_expect(focus == dansi_Event_Focus_out)) $end(pattern);
    default_() try_(TEST_expect(false)) $end(default);
    } $end(match);
    return_ok({});
} $unscoped(TEST_fn);
