...
#define SCREEN_BUFFER_SIZE (320 * 180 * 40)
typedef struct {
    char data[SCREEN_BUFFER_SIZE];
    int  position;
    int  capacity;
} ScreenBuffer;

// Double buffering
static ScreenBuffer front_buffer = { 0 };
static ScreenBuffer back_buffer  = { 0 };
...
void  render_hex_tile(ScreenBuffer* buffer, int x, int y, Tile* tile, World* world);
void  get_hex_neighbors(int x, int y, int neighbors[6][2], int* count);
void  move_cursor(World* world, char direction);
void  print_info_panel(ScreenBuffer* buffer, World* world);
...
u8    getch(void);
bool  kbhit(void);
void  init_screen_buffer(ScreenBuffer* buffer);
void  clear_screen_buffer(ScreenBuffer* buffer);
void  buffer_write_char(ScreenBuffer* buffer, char c);
void  buffer_write_string(ScreenBuffer* buffer, const char* str);
void  buffer_write_formatted(ScreenBuffer* buffer, const char* format, ...);
void  buffer_set_color_256(ScreenBuffer* buffer, int color);
...
int main() {
    ...
    srand(time(NULL));
    init_terminal();
    init_screen_buffer(&front_buffer);
    init_screen_buffer(&back_buffer);
    init_world(&world);
...
void render_hex_tile(ScreenBuffer* buffer, int x, int y, Tile* tile, World* world) {
    ...
    if (x == world->cursor_x && y == world->cursor_y) {
        buffer_write_string(buffer, "\033[7m");
    }

    buffer_set_color_256(buffer, color);
    buffer_write_char(buffer, symbol);

    if (x == world->cursor_x && y == world->cursor_y) {
        buffer_write_string(buffer, "\033[27m");
    }
}

void render_world(World* world) {
    // Clear and prepare the back buffer
    clear_screen_buffer(&back_buffer);
    buffer_write_string(&back_buffer, CLEAR);

    buffer_write_formatted(&back_buffer, "=== PlanetSim === Step:%d Day:%d ", world->time_step, world->day_cycle);
    if (world->paused) {
        buffer_write_string(&back_buffer, "[PAUSED]");
    }
    buffer_write_string(&back_buffer, "\n");

    const char* modes[] = { "Terrain", "Temperature", "Moisture", "Life",
                            "Plates", "Currents", "Winds", "O2", "Altitude" };
    buffer_write_formatted(&back_buffer, "View:%s | Temp:%.1f°C | Sea:%.0fm | Tilt:%.1f° | O2:%.1f%%\n", modes[world->view_mode], world->global_temp, world->planet.sea_level, world->planet.axial_tilt, world->atmosphere.oxygen * 100);
    buffer_write_string(&back_buffer, "\n");
    ...
    for (int y = MAP_HEIGHT - 1; y >= 0; y--) {
        ...
        for (int x = 0; x < MAP_WIDTH; x++) {
            render_hex_tile(&back_buffer, x, y, &world->tiles[y][x], world);
            buffer_write_string(&back_buffer, " ");
        }
        buffer_write_string(&back_buffer, RESET "\n");
    }

    print_info_panel(&back_buffer, world);

    // Swap buffers - this will output the back buffer and clear it
    swap_buffers();
}

void print_info_panel(ScreenBuffer* buffer, World* world) {
    buffer_write_string(buffer, "\n=== World Statistics ===\n");
    buffer_write_formatted(buffer, "Atmosphere: N2:%.1f%% O2:%.1f%% CO2:%.3f%% CH4:%.4f%%\n", world->atmosphere.nitrogen * 100, world->atmosphere.oxygen * 100, world->atmosphere.co2 * 100, world->atmosphere.methane * 100);

    Tile* t = &world->tiles[world->cursor_y][world->cursor_x];
    buffer_write_formatted(buffer, "\n=== Cursor (%d,%d) ===\n", world->cursor_x, world->cursor_y);
    buffer_write_formatted(buffer, "Altitude: %.0fm | Water: %.0fm | Plate: %d\n", t->altitude, t->water_depth, t->plate_id);
    buffer_write_formatted(buffer, "Temp: %.1f°C | Moisture: %.0f%% | Ice: %.0f%%\n", t->temperature, t->moisture * 100, t->ice_coverage * 100);

    if (t->is_lake) {
        buffer_write_string(buffer, "[LAKE] ");
    }
    if (t->ocean_current.strength > 0.01) {
        buffer_write_formatted(buffer, "Current:%.2f ", t->ocean_current.strength);
    }
    if (t->wind.strength > 0.01) {
        buffer_write_formatted(buffer, "Wind:%.2f ", t->wind.strength);
    }
    if (t->magma_heat > 0.01) {
        buffer_write_formatted(buffer, "Magma:%.0f%%", t->magma_heat * 100);
    }
    buffer_write_string(buffer, "\n");
    ...
    if (t->life != LIFE_NONE) {
        buffer_write_formatted(buffer, "Life: %s (age:%d)\n", life_names[t->life], t->age);
    }

    buffer_write_string(buffer, "\nControls: WASD/ER=move | 1-9=views | +/-=sea level | T=volcano | Space=pause\n");
}

fn_(move_cursor(World* world, char direction), void) {
    switch (direction) {
    case 'N': // North
        world->cursor_y = (world->cursor_y - 1 + MAP_HEIGHT) % MAP_HEIGHT;
        break;
    case 'S': // South
        world->cursor_y = (world->cursor_y + 1) % MAP_HEIGHT;
        break;
    case 'W': // West
        world->cursor_x = (world->cursor_x - 1 + MAP_WIDTH) % MAP_WIDTH;
        break;
    case 'E': // East
        world->cursor_x = (world->cursor_x + 1) % MAP_WIDTH;
        break;
    case 'Q': // Northwest (for hex grid)
        if (world->cursor_y % 2 == 0) {
            // Even row
            world->cursor_x = (world->cursor_x - 1 + MAP_WIDTH) % MAP_WIDTH;
        }
        world->cursor_y = (world->cursor_y - 1 + MAP_HEIGHT) % MAP_HEIGHT;
        break;
    case 'C': // Northeast (for hex grid)
        if (world->cursor_y % 2 == 1) {
            // Odd row
            world->cursor_x = (world->cursor_x + 1) % MAP_WIDTH;
        }
        world->cursor_y = (world->cursor_y - 1 + MAP_HEIGHT) % MAP_HEIGHT;
    ...
    if (world->cursor_x >= MAP_WIDTH) {
        world->cursor_x = MAP_WIDTH - 1;
    }
    if (world->cursor_y < 0) {
        world->cursor_y = 0;
    }
    if (world->cursor_y >= MAP_HEIGHT) {
        world->cursor_y = MAP_HEIGHT - 1;
    }
}

fn_(init_terminal(void), void) {
#if bti_plat_windows
    // Windows console setup for better ANSI support
...
fn_(restore_terminal(void), void) {
#if bti_plat_windows
    // Windows doesn't need explicit restoration
#else  /* bti_plat_unix */
...
// Screen buffer functions for double buffering
void init_screen_buffer(ScreenBuffer* buffer) {
    buffer->position = 0;
    buffer->capacity = SCREEN_BUFFER_SIZE - 1; // Leave space for null terminator
    buffer->data[0]  = '\0';
}
...
void buffer_write_string(ScreenBuffer* buffer, const char* str) {
    ...
    while (*str && buffer->position < buffer->capacity) {
        buffer->data[buffer->position] = *str;
        buffer->position++;
        str++;
    }
    buffer->data[buffer->position] = '\0';
...
void buffer_write_formatted(ScreenBuffer* buffer, const char* format, ...) {
    ...
    int written   = vsnprintf(buffer->data + buffer->position, remaining + 1, format, args);

    if (written > 0 && written <= remaining) {
        buffer->position += written;
    } else if (written > remaining) {
        // Truncated - move position to end
...
void buffer_set_color_256(ScreenBuffer* buffer, int color) {
    buffer_write_formatted(buffer, "\033[38;5;%dm", color);
}
...
void swap_buffers(void) {
    // Output the back buffer to screen
    flush_buffer_to_screen(&back_buffer);

    // Clear the back buffer for next frame
    clear_screen_buffer(&back_buffer);
}
