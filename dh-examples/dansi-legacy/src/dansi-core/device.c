...
$attr($must_check)
$static fn_((dansi_device__receiveReport(io_Reader in, S$u8 buf))(E$S$u8));
$attr($must_check)
...
fn_((dansi_device__receiveReport(io_Reader in, S$u8 buf))(E$S$u8)) {
    return dansi_Seq_receiveCSI(in, buf);
};

fn_((dansi_device__parseAttributesReport(
    S_const$u8 report,
    dansi_device_AttrsKind expected
    ...
    let_(kind, dansi_device_AttrsKind) = first == '>' ? dansi_device_AttrsKind_sec
                                       : first == '=' ? dansi_device_AttrsKind_ter
