...
`manifest.dh` is the fixed compatibility and artifact inventory for one
...
- `abi-contract` identifies the compiled C ABI and build facts.
...
The compile ABI identity and producer-link metadata deliberately answer different
questions.

The compile ABI identity includes the normalized target, profile, artifact
...
The producer-link metadata includes the compiler driver, resolved target,
...
is part of the producer-link metadata. It is not interpreted as "no runtime
...
of the object-cache and prebuilt ABI identity.

A custom runtime may instead expose no libc at all. Its exact libraries and
link arguments still participate in producer-link metadata. Any runtime identity
...
metadata match. LTO artifacts require the exact toolchain identity.
...
manifest targets differ. Explicit `target-abi`, sysroot, and public
...
- the compiled ABI identity matches;
...
- on Windows, a selected shared library also has its listed and present import
  library with the same recorded identity.
...
The command promotes the generated prebuilt metadata:
...
metadata. It translates library artifacts into `lib/` and runtime shared
