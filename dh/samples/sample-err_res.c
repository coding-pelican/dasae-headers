...
$attr($must_check)
$static fn_((safeDivide(i32 num, i32 denom))(my_math_E$i32) $scope) {
    // Check for division by zero
    if (denom == 0) return_err(E_cause$my_math_DivisionByZero());
    // Check for potential overflow
    if (num == i32_limit_min && denom == -1) return_err(E_cause$my_math_Overflow());
    return_ok(num / denom);
} $unscoped(fn);

// Function demonstrating error propagation with try_
T_use_E$($set(my_math_E)(f32));
$attr($must_check)
$static fn_((calculateRatio(i32 a, i32 b, i32 c, i32 d))(my_math_E$f32) $scope) {
...
T_alias$((my_math_Res)(variant_((my_math_Res $fits($packed))(
    (math_Res_i32, my_math_E$i32),
    (math_Res_f32, my_math_E$f32)
))));
...
fn_((main(S$S_const$u8 args))(E$void) $scope) {
    ...
    processResult(union_of$((my_math_Res)(math_Res_i32)(safeDivide(10, 2))));
    processResult(union_of$((my_math_Res)(math_Res_i32)(safeDivide(10, 0))));

    // Error propagation
    io_stream_println(u8_l("\nCalculating ratios:"));
    using_(var ratio_result = calculateRatio(10, 2, 6, 3)) {
        processResult(union_of$((my_math_Res)(math_Res_f32)(ratio_result)));
    }
    ...
    io_stream_println(u8_l("\nComplex operation with cleanup:"));
    processResult(union_of$((my_math_Res)(math_Res_i32)(performOperation(10, 2))));
    processResult(union_of$((my_math_Res)(math_Res_i32)(performOperation(10, 0))));
...
T_use_E$($set(my_math_E)(i32)); // or Generally `use_E$(i32)`
$attr($must_check)
$static fn_((safeDivide(i32 lhs, i32 rhs))(my_math_E$i32) $scope) {
    if (rhs == 0) {
        return_err(E_cause$my_math_DivisionByZero()); // Return with an error
    }
