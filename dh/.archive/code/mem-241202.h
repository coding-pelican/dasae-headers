...
typedef struct mem_Allocator {
    ...
    Res_anyptr (*alloc)(anyptr ctx, usize size, mem_Debug* debug);
    Res_anyptr (*realloc)(anyptr ctx, anyptr ptr, usize new_size, mem_Debug* debug);
...
extern Res_Sptr mem_createSptr(mem_Allocator* alloc, usize elem_size, usize elem_align);
extern Res_Sptr mem_createSptrDebug(mem_Allocator* alloc, usize elem_size, usize elem_align, const char* file, i32 line, const char* func);

// Destroy single element
extern void mem_Allocator_destroy(mem_Allocator* alloc, Sptr* ptr);
...
extern Res_Slice mem_allocSlice(mem_Allocator* alloc, usize elem_size, usize elem_align, usize count);
extern Res_Slice mem_allocSliceDebug(mem_Allocator* alloc, usize elem_size, usize elem_align, usize count, const char* file, i32 line, const char* func);

// Free array
extern void mem_Allocator_free(mem_Allocator* alloc, Slice* slice);

// Resize array
#define mem_realloc(alloc, slice, T, new_count) \
    mem_reallocSlice(alloc, slice, sizeof(T), alignof(T), new_count)

extern Res_Slice mem_reallocSlice(mem_Allocator* alloc, Slice slice, usize elem_size, usize elem_align, usize new_count);
