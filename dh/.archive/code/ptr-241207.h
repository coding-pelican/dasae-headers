...
force_inline anyptr Slice_rawBegin(Slice self);
force_inline anyptr Slice_rawEnd(Slice self);
force_inline Mptr   Slice_ptr(Slice self);
force_inline Mptr   Slice_ptrBegin(Slice self);
force_inline Mptr   Slice_ptrEnd(Slice self);
...
force_inline cptr Sptr_cast(cptr self);
force_inline cptr Mptr_cast(cptr self);
...
force_inline Slice Slice_make(Mptr ptr, usize len) {
    return (Slice){ .ptr_ = ptr, .len_ = len };
}

force_inline anyptr Slice_raw(Slice self) {
    return Mptr_raw(Slice_ptr(self));
}

force_inline anyptr Slice_rawBegin(Slice self) {
    return Mptr_raw(Slice_ptrBegin(self));
}

force_inline anyptr Slice_rawEnd(Slice self) {
    return Mptr_raw(Slice_ptrEnd(self));
}
...
force_inline Mptr Slice_ptrEnd(Slice self) {
    return Mptr_add(self.ptr_, as(isize, Slice_len(self)));
}

force_inline usize Slice_size(Slice self) {
    return Mptr_size(self.ptr_);
}

force_inline usize Slice_align(Slice self) {
    return Mptr_align(self.ptr_);
}
...
force_inline bool Slice_isZero(Slice self) {
    return Mptr_isZero(self.ptr_);
}

force_inline bool Slice_isAligned(Slice self) {
    return Mptr_isAligned(self.ptr_);
}

force_inline void Slice_copy(Slice dest, Slice src) {
    debug_assert_eq(dest.len_, src.len_);
    debug_assert_eq(Slice_size(dest), Slice_size(src));
    memcpy(
        (anyptr)Slice_raw(dest),
        Slice_raw(src),
        dest.len_ * Slice_size(dest)
    );
}
...
force_inline void Slice_clear(Slice self) {
    memset(
        ...
        0,
        self.len_ * Slice_size(self)
    );
}

/*========== Conversion Functions ============================================*/

force_inline cptr Sptr_cast(Sptr self) {
    return self;
}

force_inline cptr Mptr_cast(Mptr self) {
    return self;
}
