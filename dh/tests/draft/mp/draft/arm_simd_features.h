...
#ifdef __ARM_FEATURE_FP16_VECTOR_ARITHMETIC
static inline void arm_fp16_add(const __fp16* a, const __fp16* b, __fp16* c, int n) {
    int i = 0;
...
static inline void arm_vector_add_auto(const float* a, const float* b, float* c, int n) {
#ifdef __ARM_NEON
    const arm_simd_features_t* f = arm_detect_features();
...
#ifdef __ARM_NEON
static inline void arm_vector_add_unrolled(const float* a, const float* b, float* c, int n) {
    int i = 0;
...
// 4x 언롤링 (16개 동시 처리)
static inline void arm_vector_add_unrolled4x(const float* a, const float* b, float* c, int n) {
    int i = 0;
