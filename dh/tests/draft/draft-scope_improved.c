...
T_Return _Name_With_Params {                               \
    let __reserved_return = as$(T_Return*,                 \
        (u8[Generic_match$(T_Return,                   \
            Generic_pattern$(void) 0,                  \
            Generic_fallback_ sizeOf$(T_Return))]){}); \
...
#define comp_syn__return_void()         local_({                                 \
    claim_assert_static(eqlType$(TypeOf(*__reserved_return), TypeOf(void))); \
    goto __step_return;                                                        \
})
...
T_Return _Name_With_Params {                                   \
    ...
    switch (__scope_counter.current_line) {                    \
    case line():                                           \
        __scope_counter.current_line = line() - 1;
#define comp_syn__unscoped_ext                    \
    default:                                      \
        if (false) { __step_unscope:              \
            if (Generic_match$(TypeOf(void),  \
                Generic_pattern$(void) false, \
                Generic_fallback_ true)       \
            ) { return __reserved_return[0]; }    \
        } else { claim_unreachable; }             \
...
#define fn_scope(_Name_With_Params, T_Return)                   \
T_Return _Name_With_Params {                                    \
    u8          __reserved_buf[Generic_match$(TypeOf(void), \
        Generic_pattern$(void) 0,                           \
        Generic_fallback_ sizeOf(void)                      \
    )];                                                         \
...
/* #define ret_(_Expr...)                                                      \
    __reserved_return = Generic_match$(                                 \
        TypeOf(*__reserved_return),                                         \
        Generic_pattern$(void) null,                                    \
        Generic_fallback_(TypeOf(*__reserved_return)[1]){ [0] = _Expr } \
    );                                                                      \
    goto __step_return */

/* #define ret_void local_({                                                        \
    claim_assert_static(eqlType$(TypeOf(*__reserved_return), TypeOf(void))); \
    goto __step_return;                                                        \
})
#define ret_(_Expr...) local_({                                       \
    pri_memcpy(                                                     \
...
// void func(void) {
//     u8          __reserved_buf[Generic_match$(TypeOf(void), Generic_pattern$(void) 0, Generic_fallback_ sizeOf(void))];
