...
#include "draft-meta_system.c" // NOLINT
#include "dh/prl.h"
...
fn_((ArrDeq_ensureCapPrecise(ArrDeq* self, mem_Allocator gpa, usize new_capacity))(mem_Err$void) $scope) {
    ...
    if_some(mem_Allocator_remap(gpa, anySli(old_buf), new_capacity), new_buf) {
        ...
        self->head = 0;
        self->buf  = new_buf;
        mem_Allocator_free(gpa, anySli(old_buf));
    }
}
...
pub fn Deque(comptime T : type) type {
    return struct {
        ...
        pub fn ensureTotalCapacity(deque : *Self, gpa : Allocator, new_capacity : usize) Allocator.Error !void {
            if (deque.buffer.len >= new_capacity) {
                return;
            }
        ...
        pub fn ensureTotalCapacityPrecise(deque : *Self, gpa : Allocator, new_capacity : usize) Allocator.Error !void {
            ...
            if (gpa.remap(old_buffer, new_capacity)) {
                | new_buffer | {
                    // If the items wrap around the end of the buffer we need to do
                    // a prim_memcpy to prevent a gap after resizing the buffer.
                    if (deque.head > old_buffer.len - deque.len) {
                        ...
                        if (head.len > tail.len and new_buffer.len - old_buffer.len > tail.len) {
                            @prim_memcpy(new_buffer[old_buffer.len..][0..tail.len], tail);
                        } else {
                            // In this case overlap is possible if e.g. the capacity increase is 1
                            // and head.len is greater than 1.
                            deque.head = new_buffer.len - head.len;
                            @prim_memmove(new_buffer[deque.head..][0..head.len], head);
                        }
                    }
                ...
                if (deque.head < old_buffer.len - deque.len) {
                    @prim_memcpy(new_buffer[0..deque.len], old_buffer[deque.head..][0..deque.len]);
                } else {
                    const head = old_buffer[deque.head..];
                    const tail = old_buffer[0 ..deque.len - head.len];
                    @prim_memcpy(new_buffer[0..head.len], head);
                    @prim_memcpy(new_buffer[head.len..][0..tail.len], tail);
                }
                deque.head   = 0;
                deque.buffer = new_buffer;
                gpa.free(old_buffer);
        ...
        ///
        /// Invalidates element pointers if additional memory is needed.
        pub fn pushFront(deque : *Self, gpa : Allocator, item : T) error { OutOfMemory }
        !void {
        ...
        /// `error.OutOfMemory`.
        pub fn pushFrontBounded(deque : *Self, item : T) error { OutOfMemory }
        !void {
            if (deque.buffer.len - deque.len == 0) {
        ...
        ///
        /// Invalidates element pointers if additional memory is needed.
        pub fn pushBack(deque : *Self, gpa : Allocator, item : T) error { OutOfMemory }
        !void {
            try deque.ensureUnusedCapacity(gpa, 1);
        ...
        /// If the deque lacks unused capacity for the additional item, returns
        /// `error.OutOfMemory`.
        pub fn pushBackBounded(deque : *Self, item : T) error { OutOfMemory }
        !void {
            if (deque.buffer.len - deque.len == 0) {
                return error.OutOfMemory;
            }
...
fn addOrOom(a : usize, b : usize) error { OutOfMemory }
!usize {
