...
#define comp_op__variant_asg$(__addr_variant, T_Variant, var_addr_variant, val_variant...) local_({ \
    let __addr_variant = var_addr_variant; \
    debug_assert_nonnull(__addr_variant); \
    *__addr_variant = *(T_Variant[1]){ [0] = val_variant }; \
    local__return __addr_variant; \
})
#define comp_op__variant_asg(var_addr_variant, val_variant...) \
    variant_asg$(TypeOf(*var_addr_variant), var_addr_variant, val_variant)

#define comp_op__variant_as(__addr_variant, _addr_variant, _tag...) local_({ \
    let __addr_variant = _addr_variant; \
    debug_assert(__addr_variant->tag == _tag); \
    local__return __addr_variant->payload.pp_join($, tagged, _tag); \
})

#define comp_op__variant_extract(__var_variant, _var_variant, _tag...) local_({ \
    let __var_variant = _var_variant; \
    debug_assert(__var_variant.tag == _tag); \
    local__return __var_variant.payload.pp_join($, tagged, _tag)[0]; \
})
...
#define comp_syn__match_(_Variant, _Body...) local_({ \
...
// fn_(Shape_calcArea(Shape self), f32 $scope) {
//     return_(local__(area, f32, {
//         match_(self, {
//             pattern_(Shape_circ, (c), {
//                 local__break_(area, 3.1415926f * c->radius * c->radius);
//             });
//             pattern_(Shape_rect, (r), {
//                 local__break_(area, r->width * r->height);
//             });
//         });
//     }));
// } $unscoped(fn);
