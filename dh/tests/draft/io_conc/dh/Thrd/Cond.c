...
pp_if_(Thrd_Cond_use_pthread)(
    pp_then_(
        ...
        $static fn_((Thrd_Cond__pthread_timedWait(P$$(Thrd_Cond) self, P$$(Thrd_Mtx) mtx, time_Dur duration))(Thrd_Cond_E$void));
        $attr($inline_always)
    ...
    pp_else_(
        ...
        $attr($inline_always $must_check)
        $static fn_((Thrd_Cond__common_timedWait(P$$(Thrd_Cond) self, P$$(Thrd_Mtx) mtx, time_Dur duration))(Thrd_Cond_E$void));
        $attr($inline_always)
...
fn_((Thrd_Cond_timedWait(Thrd_Cond* self, Thrd_Mtx* mtx, time_Dur duration))(Thrd_Cond_E$void)) {
...
fn_((Thrd_Cond__pthread_timedWait(Thrd_Cond* self, Thrd_Mtx* mtx, time_Dur duration))(Thrd_Cond_E$void) $scope) {
    struct timespec abs_ts = cleared();
    if (clock_gettime(CLOCK_MONOTONIC, &abs_ts) != 0) {
        return_err(Thrd_Cond_E_SystemResources());
    }
    ...
    case_((ETIMEDOUT /* TIMED OUT */)) return_err(Thrd_Cond_E_Timeout()) $end(case);
...
pp_if_(Thrd_Cond_has_specialized)(
    pp_then_(pp_expand(
        pp_switch_ pp_begin(plat_type)(
            pp_case_((plat_type_windows)(
                ...
                $attr($inline_always $must_check)
                $static fn_((Thrd_Cond__windows_impl_wait(P$$(Thrd_Cond) self, P$$(Thrd_Mtx) mtx, O$time_Dur timeout))(Thrd_Cond_E$void));
                $attr($inline_always)
    ...
    pp_else_(
        ...
        $attr($inline_always $must_check)
        $static fn_((Thrd_Cond__default_impl_wait(P$$(Thrd_Cond) self, P$$(Thrd_Mtx) mtx, O$time_Dur timeout))(Thrd_Cond_E$void));
        $attr($inline_always)
...
fn_((Thrd_Cond__common_timedWait(Thrd_Cond* self, Thrd_Mtx* mtx, time_Dur duration))(Thrd_Cond_E$void)) {
...
fn_((Thrd_Cond__default_impl_wait(Thrd_Cond* self, Thrd_Mtx* mtx, O$time_Dur timeout))(Thrd_Cond_E$void)) {
...
fn_((Thrd_Cond__windows_impl_wait(Thrd_Cond* self, Thrd_Mtx* mtx, O$time_Dur timeout))(Thrd_Cond_E$void) $scope) {
    ...
    if (!rc) {
        ...
        if (!timeout_overflowed) { return_err(Thrd_Cond_E_Timeout()); }
    }
    return_ok({});
} $unscoped(fn);
