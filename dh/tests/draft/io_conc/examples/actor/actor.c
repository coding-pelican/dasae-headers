...
fn_((actor_Sys_init(exec_Coop* coop, mem_Alctr base_alctr, usize max_units))(E$actor_Sys) $scope) {
...
fn_((actor_Sys_createUnit(actor_Sys* self))(E$P$actor_Unit) $scope) {
    claim_assert_nonnull(self);
    if (self->next_id >= self->units.len) return_err(mem_E_OutOfMemory());
...
fn_((actor_Unit_start(actor_Unit* self, Closure$Void* worker))(E$void) $scope) {
...
fn_((actor_Unit_startE(actor_Unit* self, Closure$E$Void* worker))(E$void) $scope) {
    claim_assert_nonnull(self), claim_assert_nonnull(worker);
    claim_assert(isNone(self->task));
    claim_assert(isNone(self->exit));
...
fn_((actor_Unit_send(actor_Unit* self, actor_Msg msg))(E$void) $scope) {
    claim_assert_nonnull(self);
