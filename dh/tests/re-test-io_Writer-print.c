...
$static fn_((test_Buf_VT_write(const P$raw ctx, S_const$u8 bytes))(E$usize) $scope) {
    ...
    if (0 < to_write) {
        memcpy(self->data.ptr + self->pos, bytes.ptr, to_write);
        self->pos += to_write;
    }
    return_ok(to_write);
} $unscoped_(fn);
...
$static fn_((test_Buf_take(test_Buf* self, S_const$u8 data))(bool)) {
    ...
    memcpy(self->data.ptr, data.ptr, data.len);
    self->pos = data.len;
    return true;
}
$maybe_unused
$static fn_((test_Buf_give(test_Buf* self, S$u8 output))(O$S$u8) $scope) {
    ...
    memcpy(output.ptr, self->data.ptr, self->pos);
    self->pos = 0;
    return_some(output);
} $unscoped_(fn);
...
$static fn_((test_Buf_giveAlloc(test_Buf* self, mem_Allocator allocator))(E$S$u8) $scope) {
    ...
    memcpy(out.ptr, self->data.ptr, self->pos);
    self->pos = 0;
    return_ok(out);
} $unscoped_(fn);
