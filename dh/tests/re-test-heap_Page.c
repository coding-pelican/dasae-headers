...
//     heap_Page     heap_ctx  = {};
//     mem_Alctr allocator = heap_Page_alctr(&heap_ctx);

//     // // Test zero-size allocation
//     // TypeInfo zero_type    = { .size = 0, .align = 1 };
//     // let      zero_sli_res = mem_Alctr_alloc(allocator, zero_type, 1);
...
//     //     mem_Alctr_free(allocator, meta_sliToAny(zero_sli));
//     // }

//     // Test page-sized allocation
//     TypeInfo page_type = { .size = mem_page_size, .align = mem_page_size };
//     let      page_res  = mem_Alctr_create(allocator, page_type);
...
//         mem_Alctr_destroy(allocator, meta_ptrToAny(ptr));
//     }
// }
// test_unscoped;
...
TEST_Result TEST_heap_Page_Alignment(void) {
    ...
    for (usize align = 1; align <= mem_page_size; align *= 2) {
        ...
        let res = mem_Alctr_create(allocator, type);
...
TEST_Result TEST_heap_Page_Slices(void) {
    ...
    TypeInfo page_type = { .size = mem_page_size, .align = mem_page_size };
    let slice_res = mem_Alctr_alloc(allocator, page_type, 2); // 2 pages
    ...
    if (!slice_res.is_err) {
        ...
        let shrink_ok = mem_Alctr_resize(allocator, memory, 1);
        TEST_condition(shrink_ok);

        // Try expanding back (should fail - page allocator doesn't support expansion)
        let expand_not_ok = mem_Alctr_resize(allocator, memory, 3);
        TEST_condition(!expand_not_ok);

        // // Try reallocation
        // let realloc_res = mem_Alctr_realloc(allocator, memory, 3);
        // TEST_condition(!realloc_res.has_value);

        mem_Alctr_free(allocator, memory);
    }
...
TEST_Result TEST_heap_Page_LargeAlloc(void) {
    ...
    let large_res = mem_Alctr_alloc(allocator, large_type, almost_max);
    ...
    let valid_res = mem_Alctr_alloc(allocator, large_type, large_but_valid);
    ...
    if (!valid_res.is_err) {
        ...
        mem_Alctr_free(allocator, meta_sliToAny(large_slice));
    }
...
TEST_Result TEST_heap_Page_Errors(void) {
    ...
    let huge_res = mem_Alctr_alloc(allocator, type, usize_limit);
    ...
    // let      align_res    = mem_Alctr_create(allocator, over_aligned);
...
TEST_Result TEST_heap_Page_MultiPageAccess(void) {
    ...
    let res = mem_Alctr_alloc(allocator, type, num_pages);
    TEST_condition(!res.is_err);
    ...
    if (!res.is_err) {
        ...
        mem_Alctr_free(allocator, meta_sliToAny(slice));
    }
...
int main(int argc, const char* argv[]) {
    ...
    TEST_init();
    printf("Starting Page Alctr Tests\n");
