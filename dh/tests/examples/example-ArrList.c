...
$static fn_((printPoints(ArrList$Point points))(void)) {
    io_stream_print(u8_l("Points ({:zu} items):\n"), points.items.len);
    for_(($rf(0), $s(points.items))(index, point) {
        io_stream_print(u8_l("  [{:zu}] = ({:d}, {:d})\n"), index, point->x, point->y);
    });
}
...
$static fn_((example(void))(mem_Err$void) $guard) {
    ...
    var points = try_(ArrList_init$Point(allocator, 8));
    defer_(ArrList_fini$Point(&points, allocator));

    // Append a single item
    try_(ArrList_append$Point(&points, allocator, (Point){ .x = 10, .y = 20 }));

    // Append multiple items at once
    A$$(2, Point) more_points = init$A({
        { .x = 30, .y = 40 },
        { .x = 50, .y = 60 },
    });
    try_(ArrList_appendS$Point(&points, allocator, ref$A$((Point)(more_points)).as_const));
    ...
    Point p4 = { .x = 70, .y = 80 };
    try_(ArrList_insert$Point(&points, allocator, 1, p4));
    ...
    // Pop last element
    if_some((ArrList_pop$Point(&points))(item_last)) {
