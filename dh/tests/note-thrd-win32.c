...
int Thrd_setName(Thrd self, const char* name) {
    ...
    if (hKernel) {
        ...
        pSetThreadDescription fnSetDesc = (pSetThreadDescription)(void*)GetProcAddress(hKernel, "SetThreadDescription");
        if (fnSetDesc) {
...
int Thrd_getName(Thrd self, char* buffer, size_t bufferSize) {
    ...
    if (hKernel) {
        ...
        pGetThreadDescription fnGetDesc = (pGetThreadDescription)(void*)GetProcAddress(hKernel, "GetThreadDescription");
        if (fnGetDesc) {
            PWSTR   wName = NULL;
...
static int ThrdFtx_wait(ThrdFtx* ftx, LONG expect) {
    ...
    if (!rc) {
        ...
        /* spurious wake or mismatch => let_ignore = */
    }
    return 0;
}
...
int main(void) {
    ...
    let_ignore = fprintf(stdout, "All examples finished.\n");
    return 0;
}
