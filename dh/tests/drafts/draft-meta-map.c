...
#define __u_allocA__emit(__len, __type, __mem, _len, _type...) ({ \
    ...
    memset(__mem, 0, __len * __type.size); \
    lit$((u_A$raw){ .type = __type, .mem = __mem, .len = __len }); \
})
#define u_memcpyA(_dst, _src...) \
    __u_memcpyA__step(_dst, _src)
#define __u_memcpyA__step(...) \
    __u_memcpyA__emit(pp_uniqTok(dst), pp_uniqTok(src), __VA_ARGS__)
#define __u_memcpyA__emit(__dst, __src, _dst, _src...) ({ \
    const u_A$raw __dst = _dst; \
    const u_A$raw __src = _src; \
    memcpy(__dst.mem, __src.mem, __src.len * __src.type.size); \
    __dst; \
})
#define u_copyA(_src...) \
...
#define __u_copyA__emit(__src, _src...) ({ \
    const u_A$raw __src = _src; \
    u_memcpyA(u_allocA(__src.len, __src.type), __src); \
})

#define u_allocV(_type...)       ((u_V$raw)u_allocA(1, _type))
#define u_memcpyV(_dst, _src...) ((u_V$raw)u_memcpyA(_dst, _src))
...
#define __u_castV$__emit(_T, _Expr...)  (*as$((_T*)(memcpy(&lit$((_T){}), (_Expr).mem, sizeOf$(_T)))))
...
#define __u_castA$__emit(_T, _Expr...)  (*as$((_T*)(memcpy(&lit$((_T){}), (_Expr).mem, sizeOf$(_T)))))
...
#define __u_castO$__emit(_T, _Expr...)  (*as$((_T*)(memcpy(&lit$((_T){}), (_Expr).inner, sizeOf$(_T)))))
...
int main(void) {
    ...
    memcpy(some_foo.name + some_foo.name_len, " It's Foo!", 11);
    memcpy(some_p_ufoo->name + some_p_ufoo->name_len, " It's meta Foo!", 16);
    memcpy(casted_ufoo.name + casted_ufoo.name_len, " It's casted Foo!", 18);
    memcpy(casted_some_ufoo.name + casted_some_ufoo.name_len, " It's casted some Foo!", 23);
...
int main(void) {
    O$uVal o_val = ({
        ...
        memcpy(p_o_val->payload.some.addr, p_val, sizeof(Bar));
        *p_o_val;
    });
