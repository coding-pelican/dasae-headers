...
$static fn_((mp_ThrdPool_submitBatch(mp_ThrdPool* self, mp_ThrdPool_Task* tasks, usize task_count))(void)) {
    ...
    for_(($r(0, task_count))(i) {
        ...
        *atS(self->tasks, head % mp_max_task_count) = *task;
...
$static fn_((mp_parallel_for_pooled(mp_ThrdPool* pool, R range, mp_ThrdPool_TaskFn fn, u_V$raw params))(void)) {
    ...
    for_(($r(0, pool->threads.len))(t) {
        ...
        if (begin < end) {
            asgLit((atA(task_buf, task_count++))({
                .range = $r(begin, end),
                .workerFn = fn,
                .params = params
            }));
...
fn_((State_applyGravity_worker(R range, u_V$raw params))(void)) {
    ...
    for_(($s(targets))(particle) {
        ...
        if (d_sq > 0.01f) {  // 0.1*0.1
            let inv_d = 1.0 / sqrt(d_sq);  // Single sqrt + division
            ...
            for_(($r(0, 3))(dx_offset) {  // 0, 1, 2
                ...
                for (usize k = 0; k < count && k < State_max_particles_per_cell; ++k) {
                    ...
                    if (d_sq < min_d_sq && 0.001f < d_sq) {
                        let inv_d = 1.0 / sqrt(d_sq);  // Or use fast_inv_sqrt if available
                        let d = d_sq * inv_d;
                        let overlap = min_d - d;
