...
// 메시지 타입
T_alias$((MessageType)(enum_((MessageType $fits($packed))(
...
// 메시지 구조
T_alias$((Message)(variant_((Message $maps(MessageType))(
    (MessageType_increment, u64),
    (MessageType_get_value, u64),
    (MessageType_print, A$$(52, u8)),
    (MessageType_broadcast, u64),
    (MessageType_stop, u64),
))));
T_use$((Message)(P, O, E));
T_use$((P$Message)(E));

#include "dh/ListDbl.h"
T_use$((Message)(ListDbl_Link, ListDbl, ListDbl_empty, ListDbl_append));
T_alias$((Mailbox)(ListDbl$Message));

// Actor 구조
T_alias$((Actor)(struct Actor {
    var_(routine, Co_Ctx*); // 코루틴 컨텍스트
    var_(mailbox, Mailbox); // 메시지 큐
    var_(state, P$raw); // Actor 상태 (타입 소거)
    var_(id, u32); // Actor ID
    var_(active, bool);
    var_(has_messages, bool);
}));
T_use$((Actor)(P, S, E));
T_use$((P$Actor)(E));
T_use$((S$Actor)(E));
...
T_alias$((ActorSystem)(struct ActorSystem {
    var_(alctr, mem_Alctr); // 기본 allocator
    var_(arena_actor, heap_Arena); // Actor 전용 arena
    var_(arena_message, heap_Arena); // 메시지 전용 arena
    var_(actors, S$Actor); // Actor 슬라이스
    var_(next_id, u32);
}));
T_use$((ActorSystem)(E));
...
// Actor 시스템 초기화
$attr($must_check)
$static fn_((ActorSystem_init(mem_Alctr base_alctr, usize max_actors))(E$ActorSystem));
// Actor 시스템 종료 (모든 Actor/메시지 한번에 해제)
$static fn_((ActorSystem_fini(ActorSystem* self))(void));
// Actor 런타임 실행
$static fn_((ActorSystem_runLoop(ActorSystem* self, bool endless))(void));
// Actor 생성
$attr($must_check)
$static fn_((ActorSystem_spawn(ActorSystem* self, Co_Ctx* coroutine, P$raw initial_state))(E$P$Actor));

// 메시지 전송
$attr($must_check)
$static fn_((Actor_send(ActorSystem* sys, Actor* self, Message msg))(E$void));
// 메시지 수신 (시도)
$static fn_((Actor_tryRecv(ActorSystem* sys, Actor* self))(O$Message));
...
// Actor 시스템 초기화
T_use_E$($set(mem_E)(S$Actor));
T_use$((Actor)(mem_Alctr_alloc, mem_Alctr_free));
fn_((ActorSystem_init(mem_Alctr base_alctr, usize max_actors))(E$ActorSystem) $scope) {
    var arena_actor = heap_Arena_init(base_alctr);
    let arena_message = heap_Arena_init(base_alctr);
    let actors = local_({
        let gpa = heap_Arena_alctr(&arena_actor);
        let mem = try_(mem_Alctr_alloc$Actor($trace gpa, max_actors));
        for_(($s(mem))(uninit) {
            asg_l((uninit)({
                .routine = null,
                .mailbox = ListDbl_empty$Message(),
                .state = null,
                .id = 0,
                .active = false,
                .has_messages = false,
    ...
    while_(usize iteration = 0, true) {
        ...
        for_(($s(self->actors))(actor) {
            ...
            if (actor->has_messages) {
                resume_(actor->routine);
                messages_processed++;
                if (actor->routine->state == Co_State_ready) {
                    actor->active = false;
...
fn_((Actor_send(ActorSystem* sys, Actor* self, Message msg))(E$void) $scope) {
    ...
    *new_msg = msg;
    new_msg->next = null;
    // 메일박스에 추가
    ListDbl_append(&self->mailbox, new_msg);
...
fn_((main(S$S_const$u8 args))(E$void) $guard) {
    ...
    let field_mem = try_(mem_Alctr_create(actor_gpa, u_typeInfoRecordN(total_actors, field_types)));
