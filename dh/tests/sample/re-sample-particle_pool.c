...
$attr($must_check)
$static fn_((mp_ThrdPool_init(mem_Alctr gpa, usize thrd_count))(E$P$mp_ThrdPool) $scope) {
    let_(pool, mp_ThrdPool*) = u_castP$((mp_ThrdPool*)(try_((mem_Alctr_create(gpa, typeInfo$(InnerType))))));
    asg_l((pool)($init((LitType){
        $field((workers)$asg(u_castS$((FieldType)(try_(mem_Alctr_alloc(
            gpa, typeInfo$(InnerType), mp_max_task_count
        )))))),
        $field((threads)$asg(u_castS$((FieldType)(try_(mem_Alctr_alloc(
...
fn_((main(S$S_const$u8 args))(E$void) $guard) {
    ...
    let particles = u_castS$((S$Particle)(try_(mem_Alctr_alloc(gpa, typeInfo$(InnerType), State_particles))));
    defer_(mem_Alctr_free(gpa, u_anyS(particles)));
    let grid = u_castS$((S$Cell)(try_(mem_Alctr_alloc(gpa, typeInfo$(InnerType), State_grid_width * State_grid_height))));
...
fn_((State_hashPosition(m_V2f64 pos))(usize)) {
    ...
    let gx_1 = as$(usize)(pri_clamp(gx_0, 0, intCast$((isize)(State_grid_width))- 1));
    let gy_1 = as$(usize)(pri_clamp(gy_0, 0, intCast$((isize)(State_grid_height))- 1));
