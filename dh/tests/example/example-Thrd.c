...
fn_((dh_main(S$S_const$u8 args))(E$void) $guard) {
    ...
    var thrd = try_(Thrd_spawn(Thrd_SpawnConfig_default, Thrd_FnCtx_from$((demoThrd)(time_Duration_secs)).as_raw));
    defer_(report(u8_l("main"), u8_l("ret: {:d}"), Thrd_FnCtx_ret$((demoThrd)(Thrd_join(thrd)))));

    var thrd_defer = try_(Thrd_spawn(Thrd_SpawnConfig_default, Thrd_FnCtx_from$((demoThrdDefer)(time_Duration_secs)).as_raw));
