...
claim_assert_static(arch_endian_type_big < arch_endian_type_little);
claim_assert_static(arch_endian_native == arch_endian_type);
claim_assert_static(arch_endian_foreign != arch_endian_native);
claim_assert_static(
    arch_endian_foreign == arch_endian_type_big
    || arch_endian_foreign == arch_endian_type_little
);
...
TEST_fn_("Cfg: runtime value materializes builtin cfg facts" $scope) {
    ...
    try_(TEST_expect(cfg.target.arch.endian == as$(Target_Arch_Endian)(arch_endian_type)));
