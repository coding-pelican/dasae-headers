...
TEST_fn_("thrd: batch - typed completion templates and fixed capacity" $guard) {
...
TEST_fn_("thrd: queue - typed blocking and timed operations" $guard) {
    var_(chan_cancel, thrd_CancelTok_Src) = cleared();
    let chan_cancel_src = test_thrd_channel__cancelWakeable(&chan_cancel);
...
TEST_fn_("thrd: select - pollMut rotates ready arms" $guard) {
    ...
    let fair_first_result = *thrd_Select_Arm_result$test_thrd_SelectI8Pair(fair_first);
    ...
    let fair_second_result = *thrd_Select_Arm_result$test_thrd_SelectI8Pair(fair_second);
