...
#define T_use_thrd_SPMC_init$(_T...) __gen__T_use_thrd_SPMC_init$(_T)
$extern fn_((thrd_SPMC_fini(thrd_SPMC* self))(void));
#define T_use_thrd_SPMC_fini$(_T...) __gen__T_use_thrd_SPMC_fini$(_T)
...
$attr($must_check)
$extern fn_((thrd_SPMC_Tx_send(thrd_SPMC_Tx self, u_V$raw item, thrd_wait_Src cancel_src))(thrd_chan_WaitE$void));
#define T_use_thrd_SPMC_Tx_send$(_T...) __gen__T_use_thrd_SPMC_Tx_send$(_T)
$attr($must_check)
$extern fn_((thrd_SPMC_Tx_sendFor(thrd_SPMC_Tx self, u_V$raw item, thrd_wait_Src cancel_src, time_Dur dur))(thrd_chan_TimedE$void));
...
$attr($must_check)
$extern fn_((thrd_SPMC_Rx_recv(thrd_SPMC_Rx self, thrd_wait_Src cancel_src, u_V$raw ret_mem))(thrd_chan_WaitE$u_V$raw));
#define T_use_thrd_SPMC_Rx_recv$(_T...) __gen__T_use_thrd_SPMC_Rx_recv$(_T)
$attr($must_check)
$extern fn_((thrd_SPMC_Rx_recvFor(thrd_SPMC_Rx self, thrd_wait_Src cancel_src, time_Dur dur, u_V$raw ret_mem))(thrd_chan_TimedE$u_V$raw));
...
#define __gen__T_use_thrd_SPMC_init$(_T...) \
    $attr($inline_always) \
    $static fn_((tpl$(thrd_SPMC_init, _T)(S$(_T) buf))(thrd_SPMC$(_T))) { \
        return type$((thrd_SPMC$(_T))(thrd_SPMC_init(u_anyS(buf)))); \
    }
#define __gen__T_use_thrd_SPMC_fini$(_T...) \
    $attr($inline_always) \
    $static fn_((tpl$(thrd_SPMC_fini, _T)(P$$(thrd_SPMC$(_T)) self))(void)) { \
        return thrd_SPMC_fini(self->as_raw); \
    }
...
#define __gen__T_use_thrd_SPMC_Tx_send$(_T...) \
    $attr($inline_always $must_check) \
    $static fn_((tpl$(thrd_SPMC_Tx_send, _T)(thrd_SPMC_Tx$(_T) self, _T item, thrd_wait_Src cancel_src))(thrd_chan_WaitE$void)) { \
        return thrd_SPMC_Tx_send(*self.as_raw, u_anyV(item), cancel_src); \
    }
#define __gen__T_use_thrd_SPMC_Tx_sendFor$(_T...) \
    $attr($inline_always $must_check) \
    $static fn_((tpl$(thrd_SPMC_Tx_sendFor, _T)(thrd_SPMC_Tx$(_T) self, _T item, thrd_wait_Src cancel_src, time_Dur dur))(thrd_chan_TimedE$void)) { \
        return thrd_SPMC_Tx_sendFor(*self.as_raw, u_anyV(item), cancel_src, dur); \
    }
#define __gen__T_use_thrd_SPMC_Tx_sendOp$(_T...) \
    $attr($inline_always) \
    $static fn_((tpl$(thrd_SPMC_Tx_sendOp, _T)(thrd_SPMC_Tx$(_T) self, _T* item))(thrd_SPMC_Tx_SendOp)) { \
        return thrd_SPMC_Tx_sendOp(*self.as_raw, u_anyP(item)); \
    }
...
#define __gen__T_use_thrd_SPMC_Rx_recv$(_T...) \
    $attr($inline_always $must_check) \
    $static fn_((tpl$(thrd_SPMC_Rx_recv, _T)(thrd_SPMC_Rx$(_T) self, thrd_wait_Src cancel_src))(E$($set(thrd_chan_WaitE)(_T))) $scope) { \
        return_(u_castE$((ReturnType)(thrd_SPMC_Rx_recv(*self.as_raw, cancel_src, u_retV$(_T))))); \
    } $unscoped(fn)
#define __gen__T_use_thrd_SPMC_Rx_recvFor$(_T...) \
    $attr($inline_always $must_check) \
    $static fn_((tpl$(thrd_SPMC_Rx_recvFor, _T)(thrd_SPMC_Rx$(_T) self, thrd_wait_Src cancel_src, time_Dur dur))(E$($set(thrd_chan_TimedE)(_T))) $scope) { \
