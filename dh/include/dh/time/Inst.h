...
T_impl_O$(time_Inst);
/// Create a normalized instant value from seconds and nanoseconds.
$extern fn_((time_Inst_from(u64 secs, u32 nanos))(time_Inst));
/// Return whether the instant is exactly zero.
...
/// Add a duration to an instant in place, asserting that the result is representable.
...
/// Subtract a duration from an instant, asserting that the result is representable.
...
/// Subtract a duration from an instant in place, asserting that the result is representable.
...
/// Add a duration to an instant, returning none if the result overflows.
