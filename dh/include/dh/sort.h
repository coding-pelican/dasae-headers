...
$extern fn_((sort_inOrddCtx(u_S_const$raw seq, sort_OrdCtxFn ordFn, u_P_const$raw ctx))(bool));

use_Callable(sort_SwapFn, (u_V$raw lhs, u_V$raw rhs), void);
use_Callable(sort_SwapCtxFn, (u_V$raw lhs, u_V$raw rhs, u_V$raw ctx), void);

/// Callable types for index-based (context) sorting functions
use_Callable(sort_IdxOrdFn, (usize lhs, usize rhs, u_V$raw ctx), cmp_Ord);
use_Callable(sort_IdxSwapFn, (usize lhs, usize rhs, u_V$raw ctx), void);

/// Context definition for abstract sorting
typedef struct sort_IdxCtx {
    u_P_const$raw inner;
    sort_IdxOrdFn ordFn;
    sort_IdxSwapFn swapFn;
} sort_IdxCtx;
$attr($inline_always)
...
$extern fn_((sort_heapCtx(u_S$raw seq, sort_OrdCtxFn ordFn, u_P_const$raw ctx))(void));
...
$extern fn_((sort_pdqCtx(u_S$raw seq, sort_OrdCtxFn ordFn, u_P_const$raw ctx))(void));
...
$extern fn_((sort_blockCtx(u_S$raw seq, sort_OrdCtxFn ordFn, u_P_const$raw ctx))(void));
...
$extern fn_((sort_blockCtxCache(u_S$raw cache, u_S$raw seq, sort_OrdCtxFn ordFn, u_P_const$raw ctx))(u_S$raw));
/// Block Sort with Allocator: Optimal for O(K) memory constraints.
/// Uses provided allocator to allocate temporary buffer.
/// - Time Complexity: O(N log N) with reduced constant factors.
/// - Space Complexity: O(K) where K <= N.
$attr($must_check)
$extern fn_((sort_blockAlloc(mem_Allocator gpa, u_S$raw seq, sort_OrdFn ordFn))(mem_Err$u_S$raw));
$attr($must_check)
$extern fn_((sort_blockCtxAlloc(mem_Allocator gpa, u_S$raw seq, sort_OrdCtxFn ordFn, u_P_const$raw ctx))(mem_Err$u_S$raw));
...
fn_((sort_IdxCtx_ord(sort_IdxCtx self, usize lhs, usize rhs))(cmp_Ord)) {
    return invoke(self.ordFn, lhs, rhs, u_load(u_deref(self.inner)));
};
fn_((sort_IdxCtx_swap(sort_IdxCtx self, usize lhs, usize rhs))(void)) {
    return invoke(self.swapFn, lhs, rhs, u_load(u_deref(self.inner)));
};
