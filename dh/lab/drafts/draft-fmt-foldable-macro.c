...
typedef struct draft_fmt__Parsed {
    var_(tags, A$$(draft_fmt__max_args, draft_fmt__ArgTag));
    var_(literal_starts, A$$(draft_fmt__max_args, usize));
    var_(literal_lens, A$$(draft_fmt__max_args, usize));
    var_(count, usize);
...
#define __draft_fmt__formatTuple_3(_mem, _fmt, _arg0...) ({ \
    ...
    var_(__written, usize) = 0; \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, *A_at((__parsed.literal_starts)[0]), *A_at((__parsed.literal_lens)[0])); \
...
#define __draft_fmt__formatTuple_4(_mem, _fmt, _arg0, _arg1...) ({ \
    ...
    claim_assert(TypeInfo_eql(__tuple_meta.type, u_typeInfoRecord(__ty_fields))); \
    let __parsed = draft_fmt__parse(__fmt); \
    claim_assert(__parsed.count == 2); \
    var_(__written, usize) = 0; \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, *A_at((__parsed.literal_starts)[0]), *A_at((__parsed.literal_lens)[0])); \
    draft_fmt__writeArg(__mem, &__written, *A_at((__parsed.tags)[0]), __tuple.$0); \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, *A_at((__parsed.literal_starts)[1]), *A_at((__parsed.literal_lens)[1])); \
    draft_fmt__writeArg(__mem, &__written, *A_at((__parsed.tags)[1]), __tuple.$1); \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, __parsed.trailing_start, __parsed.trailing_len); \
    (E$S$u8)ok(S_prefix((__mem)__written)); \
})
...
#define __draft_fmt__formatTuple_5(_mem, _fmt, _arg0, _arg1, _arg2...) ({ \
    ...
    claim_assert(TypeInfo_eql(__tuple_meta.type, u_typeInfoRecord(__ty_fields))); \
    let __parsed = draft_fmt__parse(__fmt); \
    claim_assert(__parsed.count == 3); \
    var_(__written, usize) = 0; \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, *A_at((__parsed.literal_starts)[0]), *A_at((__parsed.literal_lens)[0])); \
    draft_fmt__writeArg(__mem, &__written, *A_at((__parsed.tags)[0]), __tuple.$0); \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, *A_at((__parsed.literal_starts)[1]), *A_at((__parsed.literal_lens)[1])); \
    draft_fmt__writeArg(__mem, &__written, *A_at((__parsed.tags)[1]), __tuple.$1); \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, *A_at((__parsed.literal_starts)[2]), *A_at((__parsed.literal_lens)[2])); \
    draft_fmt__writeArg(__mem, &__written, *A_at((__parsed.tags)[2]), __tuple.$2); \
    draft_fmt__copyLiteral(__mem, &__written, __fmt, __parsed.trailing_start, __parsed.trailing_len); \
    (E$S$u8)ok(S_prefix((__mem)__written)); \
})
