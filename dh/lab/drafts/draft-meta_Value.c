...
#define toSomeptr(var_addr_opt, val_some...) local_({ \
    let __addr_opt = var_addr_opt; \
    debug_assert_nonnull(__addr_opt); \
    *__addr_opt = someptr$(TypeOf(*__addr_opt), val_some); \
    local__return __addr_opt; \
})
#define toNoneptr(var_addr_opt...) local_({ \
    let __addr_opt = var_addr_opt; \
    debug_assert_nonnull(__addr_opt); \
...
#define unwrapptr(var_ptr) local_({ \
    let __ptr = var_ptr; \
    debug_assert_nonnull(__ptr.addr); \
    local__return __ptr.addr; \
})
#define orelseptr(var_ptr, val_orelse) local_({ \
    let __ptr = var_ptr; \
    local__return __ptr.addr ? __ptr.addr : val_orelse; \
})
...
fn_((Node_init(Node* self, meta_P$raw ptr))(Node*)) {
    ...
    pri_memcpy(self->data.data, ptr.ptr, ptr.type.size);
    return self;
}
...
#define meta_Value_load$(T, _self...) local_({ \
    union { \
        meta_Value meta; \
        struct { \
            meta_P$raw ref; \
    ...
    debug_assert(TypeInfo_eql(typeInfo$(T), __self.meta.ref.type)); \
    __self.value.data = deref(as$($P$(T)()(__self.meta.ref.addr))); \
    local__return __self.value.data; \
})
...
$release_only(fn_((dh_main(S$S_const$u8 args))(E$void) $scope)) {
    ...
    var_type(node, Node$i32*, Node_init(from$(Node$i32).base, meta_create$(i32, 123)));
...
} $TEST_only($unscoped(TEST_fn))
$release_only($unscoped(fn));
// clang-format on
