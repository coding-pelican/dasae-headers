...
fn_scope(io_Writer_writeAll(io_Writer self, Sli_const$u8 bytes), Err$void) {
    usize index = 0;
...
fn_scope(io_Writer_writeByteNTimes(io_Writer self, u8 byte, usize n), Err$void) {
    use_Arr$(256, u8);
...
fn_scope(io_Writer_writeBytesNTimes(io_Writer self, Sli_const$u8 bytes, usize n), Err$void) {
