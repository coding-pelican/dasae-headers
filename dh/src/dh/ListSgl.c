...
fn_((ListSgl_Link_findLast(P$ListSgl_Link self))(P$ListSgl_Link)) {
    ...
    while_some(curr->next, next) {
        debug_assert_eqBy($typed(curr->type), $typed(next->type), TypeInfo_eql);
        curr = next;
    }
    return curr;
};
...
fn_((ListSgl_prepend(ListSgl* self, P$ListSgl_Link before))(void)) {
    ...
    debug_assert_eqBy($typed(self->type), $typed(before->type), TypeInfo_eql);
...
fn_((ListSgl_remove(ListSgl* self, P$ListSgl_Link link))(void)) {
    ...
    if_some((self->first)(first)) {
        if (first == link) {
            debug_assert_eqBy($typed(first->type), $typed(link->type), TypeInfo_eql);
        ...
        while_some(current->next, next) {
            if (next == link) {
                debug_assert_eqBy($typed(current->type), $typed(next->type), TypeInfo_eql);
...
fn_((ListSgl_shift(ListSgl* self))(O$P$ListSgl_Link) $scope) {
    ...
    debug_assert_eqBy($typed(self->type), $typed(first->type), TypeInfo_eql);
