...
fn_((io_Buf_Reader_require(io_Buf_Reader* self, usize min_len))(io_ReadE$void) $scope) {
...
fn_((io_Buf_Reader_peekByte(io_Buf_Reader* self))(io_ReadE$u8) $scope) {
    try_(io_Buf_Reader_require(self, 1));
    return_ok(*S_at((self->buf)[self->start]));
} $unscoped(fn);

fn_((io_Buf_Reader_peekBytes(io_Buf_Reader* self, usize len))(io_ReadE$S_const$u8) $scope) {
    try_(io_Buf_Reader_require(self, len));
    return_ok(S_prefix((S_suffix((self->buf)(self->start)))(len)).as_const);
} $unscoped(fn);

fn_((io_Buf_Reader_takeByte(io_Buf_Reader* self))(io_ReadE$u8) $scope) {
    let byte = try_(io_Buf_Reader_peekByte(self));
    self->start += 1;
    return_ok(byte);
} $unscoped(fn);

fn_((io_Buf_Reader_takeBytes(io_Buf_Reader* self, usize len))(io_ReadE$S_const$u8) $scope) {
    let bytes = try_(io_Buf_Reader_peekBytes(self, len));
    self->start += len;
    return_ok(bytes);
} $unscoped(fn);

fn_((io_Buf_Reader_skip(io_Buf_Reader* self, usize len))(io_ReadE$void) $scope) {
...
fn_((io_Buf_Reader_readUntilByte(io_Buf_Reader* self, u8 delim, S$u8 out_buf))(io_ReadE$S$u8) $scope) {
    var_(delims, A$$(1, u8)) = A_init({ delim });
    return io_Buf_Reader_readUntilAny(self, A_ref$((S_const$u8)(delims)), out_buf);
} $unscoped(fn);

fn_((io_Buf_Reader_readUntilSeq(io_Buf_Reader* self, S_const$u8 delim, S$u8 out_buf))(io_ReadE$S$u8) $scope) {
...
fn_((io_Buf_Reader_readUntilAny(io_Buf_Reader* self, S_const$u8 delims, S$u8 out_buf))(io_ReadE$S$u8) $scope) {
...
fn_((io_Buf_Reader_skipUntilByte(io_Buf_Reader* self, u8 delim))(io_ReadE$void) $scope) {
    var_(delims, A$$(1, u8)) = A_init({ delim });
    return io_Buf_Reader_skipUntilAny(self, A_ref$((S_const$u8)(delims)));
} $unscoped(fn);

fn_((io_Buf_Reader_skipUntilSeq(io_Buf_Reader* self, S_const$u8 delim))(io_ReadE$void) $scope) {
    claim_assert_nonnullS(delim);
    claim_assert(delim.len != 0);
    if (self->buf.len < delim.len) {
        return_err(E_cause$TooSmallBuffer());
...
fn_((io_Buf_Reader_skipUntilAny(io_Buf_Reader* self, S_const$u8 delims))(io_ReadE$void) $scope) {
    claim_assert_nonnullS(delims);
    claim_assert(delims.len != 0);
    while (true) {
        if (self->end <= self->start) {
