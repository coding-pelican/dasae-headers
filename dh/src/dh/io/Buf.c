...
fn_((io_Buf_Reader_peekBytes(io_Buf_Reader* self, usize len))(E$S_const$u8) $scope) {
    try_(io_Buf_Reader_require(self, len));
    return_ok(S_prefix((S_suffix((self->buf)(self->start)))(len)).as_const);
} $unscoped(fn);

fn_((io_Buf_Reader_takeByte(io_Buf_Reader* self))(E$u8) $scope) {
    let byte = try_(io_Buf_Reader_peekByte(self));
    self->start += 1;
    return_ok(byte);
} $unscoped(fn);

fn_((io_Buf_Reader_takeBytes(io_Buf_Reader* self, usize len))(E$S_const$u8) $scope) {
    let bytes = try_(io_Buf_Reader_peekBytes(self, len));
    self->start += len;
    return_ok(bytes);
} $unscoped(fn);

fn_((io_Buf_Reader_skip(io_Buf_Reader* self, usize len))(E$void) $scope) {
...
fn_((io_Buf_Reader_readUntilSeq(io_Buf_Reader* self, S_const$u8 delim, S$u8 out_buf))(E$S$u8) $scope) {
...
fn_((io_Buf_Reader_readUntilAny(io_Buf_Reader* self, S_const$u8 delims, S$u8 out_buf))(E$S$u8) $scope) {
...
fn_((io_Buf_Reader_skipUntilSeq(io_Buf_Reader* self, S_const$u8 delim))(E$void) $scope) {
    claim_assert_nonnullS(delim);
    claim_assert(delim.len != 0);
    if (self->buf.len < delim.len) {
        return_err(E_cause$TooSmallBuffer());
...
fn_((io_Buf_Reader_skipUntilAny(io_Buf_Reader* self, S_const$u8 delims))(E$void) $scope) {
    claim_assert_nonnullS(delims);
    claim_assert(delims.len != 0);
    while (true) {
        if (self->end <= self->start) {
