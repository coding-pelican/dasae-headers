...
$static fn_((time_Proc_direct__unsupported_resoln(P$raw ctx))(time_ResolnE$time_Resoln));
pp_if_(plat_is_windows)(pp_then_(
    $static fn_((time_Proc_direct__windows_now(P$raw ctx))(time_Proc_Inst));
    $attr($must_check)
    $static fn_((time_Proc_direct__windows_resoln(P$raw ctx))(time_ResolnE$time_Resoln));
));
pp_if_(plat_based_unix)(pp_then_(
    $static fn_((time_Proc_direct__unix_now(P$raw ctx))(time_Proc_Inst));
    $attr($must_check)
    $static fn_((time_Proc_direct__unix_resoln(P$raw ctx))(time_ResolnE$time_Resoln));
));
...
$static let time_Proc_direct__resoln = pp_if_(plat_is_windows)(
    pp_then_(time_Proc_direct__windows_resoln),
    pp_else_(pp_if_(plat_based_unix)(
        pp_then_(time_Proc_direct__unix_resoln),
        pp_else_(time_Proc_direct__unsupported_resoln)
    )));
...
let_(time_Proc_VTbl_noop, time_Proc_VTbl) = {
    ...
    .resolnFn = time_Proc_VTbl_failingResoln,
};
...
$static var_(time_Proc__ctx_noop, Void) = cleared();
let_(time_Proc_noop, time_Proc) = {
    .ctx = &time_Proc__ctx_noop,
    .vtbl = &time_Proc_VTbl_noop,
};

$static var_(time_Proc__ctx_failing, Void) = cleared();
let_(time_Proc_failing, time_Proc) = {
    .ctx = &time_Proc__ctx_failing,
    ...
    pp_if_(time_Proc_direct_supported)(
        pp_then_({
            ...
            $static let_(vtbl, time_Proc_VTbl) $like_ref = { {
                ...
                .resolnFn = time_Proc_direct__resoln,
            } };
...
fn_((time_Proc_resoln(time_Proc self))(time_ResolnE$time_Resoln)) {
    self = time_Proc_ensureValid(self);
...
fn_((time_Proc_VTbl_failingResoln(P$raw ctx))(time_ResolnE$time_Resoln) $scope) {
    let_ignore = ctx;
    return_err(E_cause$time_direct_Unsupported());
} $unscoped(fn);
...
fn_((time_Proc_direct__unsupported_resoln(P$raw ctx))(time_ResolnE$time_Resoln) $scope) {
    let_ignore = ctx;
    return_err(E_cause$time_direct_Unsupported());
} $unscoped(fn);
...
pp_if_(plat_is_windows)(pp_then_(
    ...
    fn_((time_Proc_direct__windows_resoln(P$raw ctx))(time_ResolnE$time_Resoln) $scope) {
        let_ignore = ctx;
...
fn_((time_Proc_direct__unix_resoln(P$raw ctx))(time_ResolnE$time_Resoln) $scope) {
