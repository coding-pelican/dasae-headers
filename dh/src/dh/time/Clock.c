...
fn_((time_Clock_now(time_Clock clock))(time_Clock_Inst) $scope) {
    match_(clock) {
    patt_((time_Clock_real)(time)) return_(union_of((time_Clock_real)(time_Real_now(time)))) $end(patt);
    patt_((time_Clock_awake)(time)) return_(union_of((time_Clock_awake)(time_Awake_now(time)))) $end(patt);
    patt_((time_Clock_boot)(time)) return_(union_of((time_Clock_boot)(time_Boot_now(time)))) $end(patt);
    patt_((time_Clock_proc)(time)) return_(union_of((time_Clock_proc)(time_Proc_now(time)))) $end(patt);
...
fn_((time_Clock_sleep(time_Clock self, time_Dur dur))(Sched_Cancelable$void) $scope) {
    match_(self) {
    patt_((time_Clock_real)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_awake)(time)) return_(time_Awake_sleep(time, dur)) $end(patt);
    patt_((time_Clock_boot)(time)) return_(time_Boot_sleep(time, dur)) $end(patt);
    patt_((time_Clock_proc)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_thrd)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    } $end(match);
} $unscoped(fn);
fn_((time_Clock_sleepSecs(time_Clock self, u64 secs))(Sched_Cancelable$void) $scope) {
    match_(self) {
    patt_((time_Clock_real)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_awake)(time)) return_(time_Awake_sleepSecs(time, secs)) $end(patt);
    patt_((time_Clock_boot)(time)) return_(time_Boot_sleepSecs(time, secs)) $end(patt);
    patt_((time_Clock_proc)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_thrd)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    } $end(match);
} $unscoped(fn);
fn_((time_Clock_sleepMillis(time_Clock self, u64 millis))(Sched_Cancelable$void) $scope) {
    match_(self) {
    patt_((time_Clock_real)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_awake)(time)) return_(time_Awake_sleepMillis(time, millis)) $end(patt);
    patt_((time_Clock_boot)(time)) return_(time_Boot_sleepMillis(time, millis)) $end(patt);
    patt_((time_Clock_proc)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_thrd)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    } $end(match);
} $unscoped(fn);
fn_((time_Clock_sleepMicros(time_Clock self, u64 micros))(Sched_Cancelable$void) $scope) {
    match_(self) {
    patt_((time_Clock_real)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_awake)(time)) return_(time_Awake_sleepMicros(time, micros)) $end(patt);
    patt_((time_Clock_boot)(time)) return_(time_Boot_sleepMicros(time, micros)) $end(patt);
    patt_((time_Clock_proc)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_thrd)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    } $end(match);
} $unscoped(fn);
fn_((time_Clock_sleepNanos(time_Clock self, u32 nanos))(Sched_Cancelable$void) $scope) {
    match_(self) {
    patt_((time_Clock_real)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_awake)(time)) return_(time_Awake_sleepNanos(time, nanos)) $end(patt);
    patt_((time_Clock_boot)(time)) return_(time_Boot_sleepNanos(time, nanos)) $end(patt);
    patt_((time_Clock_proc)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    patt_((time_Clock_thrd)($ignore)) return_err(E_cause$Sched_Canceled()) $end(patt);
    } $end(match);
} $unscoped(fn);
...
cmp_fn_ord$((time_Clock_Inst)(lhs, rhs)) {
    match_(lhs) {
    patt_((time_Clock_real)(lhs)) return cmp_ord$(time_Real_Inst)(
        lhs, union_to((rhs)(time_Clock_real))
    ) $end(patt);
    patt_((time_Clock_awake)(lhs)) return cmp_ord$(time_Awake_Inst)(
        lhs, union_to((rhs)(time_Clock_awake))
    ) $end(patt);
    patt_((time_Clock_boot)(lhs)) return cmp_ord$(time_Boot_Inst)(
        lhs, union_to((rhs)(time_Clock_boot))
    ) $end(patt);
    patt_((time_Clock_proc)(lhs)) return cmp_ord$(time_Proc_Inst)(
        lhs, union_to((rhs)(time_Clock_proc))
    ) $end(patt);
    patt_((time_Clock_thrd)(lhs)) return cmp_ord$(time_Thrd_Inst)(
...
cmp_fn_ordCtx$((time_Clock_Inst)(lhs, rhs, ctx)) {
    match_(lhs) {
    patt_((time_Clock_real)(lhs)) return cmp_ordCtx$(time_Real_Inst)(
        lhs, union_to((rhs)(time_Clock_real)), ctx
    ) $end(patt);
    patt_((time_Clock_awake)(lhs)) return cmp_ordCtx$(time_Awake_Inst)(
        lhs, union_to((rhs)(time_Clock_awake)), ctx
    ) $end(patt);
    patt_((time_Clock_boot)(lhs)) return cmp_ordCtx$(time_Boot_Inst)(
        lhs, union_to((rhs)(time_Clock_boot)), ctx
    ) $end(patt);
    patt_((time_Clock_proc)(lhs)) return cmp_ordCtx$(time_Proc_Inst)(
        lhs, union_to((rhs)(time_Clock_proc)), ctx
    ) $end(patt);
    patt_((time_Clock_thrd)(lhs)) return cmp_ordCtx$(time_Thrd_Inst)(
...
cmp_fn_eql$((time_Clock_Inst)(lhs, rhs)) {
    match_(lhs) {
    patt_((time_Clock_real)(lhs)) return cmp_eql$(time_Real_Inst)(
        lhs, union_to((rhs)(time_Clock_real))
    ) $end(patt);
    patt_((time_Clock_awake)(lhs)) return cmp_eql$(time_Awake_Inst)(
        lhs, union_to((rhs)(time_Clock_awake))
    ) $end(patt);
    patt_((time_Clock_boot)(lhs)) return cmp_eql$(time_Boot_Inst)(
        lhs, union_to((rhs)(time_Clock_boot))
    ) $end(patt);
    patt_((time_Clock_proc)(lhs)) return cmp_eql$(time_Proc_Inst)(
        lhs, union_to((rhs)(time_Clock_proc))
    ) $end(patt);
    patt_((time_Clock_thrd)(lhs)) return cmp_eql$(time_Thrd_Inst)(
        lhs, union_to((rhs)(time_Clock_thrd))
    ) $end(patt);
    } $end(match);
};
...
cmp_fn_eqlCtx$((time_Clock_Inst)(lhs, rhs, ctx)) {
    match_(lhs) {
    patt_((time_Clock_real)(lhs)) return cmp_eqlCtx$(time_Real_Inst)(
        lhs, union_to((rhs)(time_Clock_real)), ctx
    ) $end(patt);
    patt_((time_Clock_awake)(lhs)) return cmp_eqlCtx$(time_Awake_Inst)(
        lhs, union_to((rhs)(time_Clock_awake)), ctx
    ) $end(patt);
    patt_((time_Clock_boot)(lhs)) return cmp_eqlCtx$(time_Boot_Inst)(
        lhs, union_to((rhs)(time_Clock_boot)), ctx
    ) $end(patt);
    patt_((time_Clock_proc)(lhs)) return cmp_eqlCtx$(time_Proc_Inst)(
        lhs, union_to((rhs)(time_Clock_proc)), ctx
    ) $end(patt);
    patt_((time_Clock_thrd)(lhs)) return cmp_eqlCtx$(time_Thrd_Inst)(
        lhs, union_to((rhs)(time_Clock_thrd)), ctx
    ) $end(patt);
    } $end(match);
};
