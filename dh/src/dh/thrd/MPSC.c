...
$static fn_((thrd_MPSC__sendOpPoll(P$raw ctx))(bool));
$static fn_((thrd_MPSC__sendOpCommit(P$raw ctx, u_P$raw data, u_P$raw result))(bool));
$static fn_((thrd_MPSC__recvOpPoll(P$raw ctx))(bool));
$static fn_((thrd_MPSC__recvOpCommit(P$raw ctx, u_P$raw data, u_P$raw result))(bool));

$static let_(thrd_MPSC__send_vtbl, thrd_Wakeable_VTbl) = {
    .readyFn = thrd_MPSC__sendReady,
    .linkFn = thrd_MPSC__sendLink,
    .unlinkFn = thrd_MPSC__sendUnlink,
};

$static let_(thrd_MPSC__recv_vtbl, thrd_Wakeable_VTbl) = {
...
$static let_(thrd_MPSC__send_op_vtbl, thrd_Select_Op_VTbl) = {
    ...
    .commitFn = thrd_MPSC__sendOpCommit,
};

$static let_(thrd_MPSC__recv_op_vtbl, thrd_Select_Op_VTbl) = {
    .pollFn = thrd_MPSC__recvOpPoll,
    .commitFn = thrd_MPSC__recvOpCommit,
};

fn_((thrd_MPSC_initFixed(u_S$raw buf))(thrd_MPSC)) {
...
fn_((thrd_MPSC_finiFixed(thrd_MPSC* self))(void)) {
...
fn_((thrd_MPSC_Tx_send(thrd_MPSC_Tx self, u_V$raw item, thrd_Wakeable cancel_src))(thrd_chan_WaitE$void) $guard) {
    debug_assert_eqBy($typed(self.chan->type), item.inner_type, TypeInfo_eql);
...
fn_((thrd_MPSC_Tx_sendFor(thrd_MPSC_Tx self, u_V$raw item, thrd_Wakeable cancel_src, time_Dur dur))(thrd_chan_TimedE$void) $guard) {
    debug_assert_eqBy($typed(self.chan->type), item.inner_type, TypeInfo_eql);
...
fn_((thrd_MPSC_Rx_recv(thrd_MPSC_Rx self, thrd_Wakeable cancel_src, u_V$raw out))(thrd_chan_WaitE$u_V$raw) $guard) {
...
fn_((thrd_MPSC_Rx_recvFor(thrd_MPSC_Rx self, thrd_Wakeable cancel_src, time_Dur dur, u_V$raw out))(thrd_chan_TimedE$u_V$raw) $guard) {
    debug_assert_eqBy($typed(self.chan->type), out.inner_type, TypeInfo_eql);
    let clock = catch_((time_Awake_direct())($ignore, time_Awake_noop));
    let started = time_Awake_now(clock);
    var waiter = thrd_Waiter_init();
...
fn_((thrd_MPSC_Rx_recvOp(thrd_MPSC_Rx self, TypeInfo type))(thrd_MPSC_Rx_RecvOp)) {
    ...
    return (thrd_MPSC_Rx_RecvOp){ .rx = self, .type = type };
};
...
fn_((thrd_MPSC__recvOpPoll(P$raw ctx))(bool) $scope) {
    let self = ptrAlignCast$((thrd_MPSC*)(ctx));
    return !thrd_MPSC_isEmpty(self);
} $unscoped(fn);

fn_((thrd_MPSC__recvOpCommit(P$raw ctx, u_P$raw data, u_P$raw result))(bool) $scope) {
    let self = ptrAlignCast$((thrd_MPSC*)(ctx));
    let_ignore = data;
    debug_assert_eqBy($typed(self->type), result.type, TypeInfo_eql);
    return isOk(thrd_MPSC_Rx_tryRecv((thrd_MPSC_Rx){ .chan = self }, (u_V$raw){ .inner = result.raw, .type = result.type }));
} $unscoped(fn);

fn_((thrd_MPSC__sendOpPoll(P$raw ctx))(bool) $scope) {
    let self = ptrAlignCast$((thrd_MPSC*)(ctx));
    return !thrd_MPSC_isFull(self);
} $unscoped(fn);

fn_((thrd_MPSC__sendOpCommit(P$raw ctx, u_P$raw data, u_P$raw result))(bool) $scope) {
    let self = ptrAlignCast$((thrd_MPSC*)(ctx));
    let_ignore = result;
    return isOk(thrd_MPSC_Tx_trySend((thrd_MPSC_Tx){ .chan = self }, (u_V$raw){ .inner = data.raw, .type = data.type }));
} $unscoped(fn);
