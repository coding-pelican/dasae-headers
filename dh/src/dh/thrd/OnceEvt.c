...
$static let_(thrd_OnceEvt__src_vtbl, thrd_Wakeable_VTbl) = {
...
fn_((thrd_OnceEvt_initSet(void))(thrd_OnceEvt)) {
    return (thrd_OnceEvt){
...
fn_((thrd_OnceEvt_fini(thrd_OnceEvt* self))(void)) {
    ...
    atom_V_store(&self->state, thrd_OnceEvt_State_unset, atom_MemOrd_monotonic);
};
...
fn_((thrd_OnceEvt_isSet(const thrd_OnceEvt* self))(bool)) {
    return atom_V_load(&self->state, atom_MemOrd_acquire) == thrd_OnceEvt_State_set;
};
fn_((thrd_OnceEvt_tryWait(thrd_OnceEvt* self))(bool)) {
    return thrd_OnceEvt_isSet(self);
};
fn_((thrd_OnceEvt_wait(
    ...
    let src = thrd_OnceEvt_wakeable(self);
...
fn_((thrd_OnceEvt_wakeable(thrd_OnceEvt* self))(thrd_Wakeable)) {
...
fn_((thrd_OnceEvt_set(thrd_OnceEvt* self))(void)) {
    if (atom_V_load(&self->state, atom_MemOrd_monotonic) == thrd_OnceEvt_State_set) {
        return;
    }
    thrd_Mtx_lockProtcd(&self->lock);
    let prev_state = atom_V_fetchXchg(&self->state, thrd_OnceEvt_State_set, atom_MemOrd_release);
    var waiters = self->waiters;
    self->waiters = thrd_wait_Chain_init();
    thrd_Mtx_unlock(&self->lock);
    if (prev_state == thrd_OnceEvt_State_waiting) {
...
fn_((thrd_OnceEvt_Tok_wakeable(thrd_OnceEvt_Tok self))(thrd_Wakeable)) {
    return thrd_OnceEvt_wakeable(self.event);
};
...
fn_((thrd_OnceEvt__waitUntilSet(thrd_OnceEvt* self, O$time_Dur timeout))(thrd_ftx_E$void) $scope) {
    ...
    if (state == thrd_OnceEvt_State_unset) {
        state = orelse_((atom_V_cmpXchgStrong$(
            O$u32, &self->state, state, thrd_OnceEvt_State_waiting, atom_MemOrd_acquire, atom_MemOrd_acquire
        ))(thrd_OnceEvt_State_waiting));
    }
    if (state == thrd_OnceEvt_State_waiting) {
        var deadline = thrd_ftx_Deadline_init(timeout);
        while (true) {
            let waiting = thrd_ftx_Deadline_wait(&deadline, &self->state, thrd_OnceEvt_State_waiting, none$((O$thrd_Wakeable)));
            state = atom_V_load(&self->state, atom_MemOrd_acquire);
            if (state != thrd_OnceEvt_State_waiting) { break; }
            try_(waiting);
        }
    }
    claim_assert(state == thrd_OnceEvt_State_set);
    return_ok({});
} $unscoped(fn);
