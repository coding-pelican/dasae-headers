...
fn_((thrd_RWLock_fini(thrd_RWLock* self))(void)) {
    pthread_rwlock_destroy(&self->_impl);
};

fn_((thrd_RWLock_tryLock(thrd_RWLock* self))(bool)) {
    return pthread_rwlock_trywrlock(&self->_impl) == (/*SUCCESS*/ 0);
};
...
fn_((thrd_RWLock_lockProtcd(thrd_RWLock* self))(void)) {
    let rc = pthread_rwlock_wrlock(&self->_impl);
    claim_assert(rc == (/*SUCCESS*/ 0));
};
fn_((thrd_RWLock_unlock(thrd_RWLock* self))(void)) {
    let rc = pthread_rwlock_unlock(&self->_impl);
    claim_assert(rc == (/*SUCCESS*/ 0));
};

fn_((thrd_RWLock_tryLockShared(thrd_RWLock* self))(bool)) {
    return pthread_rwlock_tryrdlock(&self->_impl) == (/*SUCCESS*/ 0);
};
...
fn_((thrd_RWLock_lockSharedProtcd(thrd_RWLock* self))(void)) {
    let rc = pthread_rwlock_rdlock(&self->_impl);
    claim_assert(rc == (/*SUCCESS*/ 0));
};
...
fn_((thrd_RWLock_fini(thrd_RWLock* self))(void)) {
    self->_impl.state = 0;
    thrd_Cond_fini(&self->_impl.cond);
    thrd_Mtx_fini(&self->_impl.mtx);
};

fn_((thrd_RWLock_tryLock(thrd_RWLock* self))(bool)) {
    if (thrd_Mtx_tryLock(&self->_impl.mtx)) {
        if ((self->_impl.state & (thrd_RWLock__is_writing | thrd_RWLock__reader_mask)) == 0) {
            self->_impl.state |= thrd_RWLock__is_writing;
            thrd_Mtx_unlock(&self->_impl.mtx);
            return true;
        }
        thrd_Mtx_unlock(&self->_impl.mtx);
    }
...
fn_((thrd_RWLock_lock(thrd_RWLock* self, thrd_Wakeable cancel_src))(Sched_Cancelable$void) $guard) {
    try_(thrd_Mtx_lock(&self->_impl.mtx, cancel_src));
    self->_impl.state += thrd_RWLock__writer;
    while ((self->_impl.state & (thrd_RWLock__is_writing | thrd_RWLock__reader_mask)) != 0) {
        catch_((thrd_Cond_wait(&self->_impl.cond, &self->_impl.mtx, cancel_src))(err, {
            self->_impl.state -= thrd_RWLock__writer;
            thrd_Cond_broadcast(&self->_impl.cond);
            thrd_Mtx_unlock(&self->_impl.mtx);
            return_err(err);
        }));
    }
    self->_impl.state += thrd_RWLock__is_writing - thrd_RWLock__writer;
    thrd_Mtx_unlock(&self->_impl.mtx);
    return_ok({});
} $unguarded(fn);
fn_((thrd_RWLock_lockProtcd(thrd_RWLock* self))(void)) {
    thrd_Mtx_lockProtcd(&self->_impl.mtx);
    self->_impl.state += thrd_RWLock__writer;
    while ((self->_impl.state & (thrd_RWLock__is_writing | thrd_RWLock__reader_mask)) != 0) {
        thrd_Cond_waitProtcd(&self->_impl.cond, &self->_impl.mtx);
    }
    self->_impl.state += thrd_RWLock__is_writing - thrd_RWLock__writer;
    thrd_Mtx_unlock(&self->_impl.mtx);
};
...
fn_((thrd_RWLock_tryLockShared(thrd_RWLock* self))(bool)) {
    if (thrd_Mtx_tryLock(&self->_impl.mtx)) {
        if ((self->_impl.state & (thrd_RWLock__is_writing | thrd_RWLock__writer_mask)) == 0) {
            self->_impl.state += thrd_RWLock__reader;
            thrd_Mtx_unlock(&self->_impl.mtx);
            return true;
        }
        thrd_Mtx_unlock(&self->_impl.mtx);
...
fn_((thrd_RWLock_lockShared(thrd_RWLock* self, thrd_Wakeable cancel_src))(Sched_Cancelable$void) $guard) {
    try_(thrd_Mtx_lock(&self->_impl.mtx, cancel_src));
    while ((self->_impl.state & (thrd_RWLock__is_writing | thrd_RWLock__writer_mask)) != 0) {
        catch_((thrd_Cond_wait(&self->_impl.cond, &self->_impl.mtx, cancel_src))(err, {
            thrd_Mtx_unlock(&self->_impl.mtx);
            return_err(err);
        }));
...
fn_((thrd_RWLock_unlockShared(thrd_RWLock* self))(void)) {
    thrd_Mtx_lockProtcd(&self->_impl.mtx);
    self->_impl.state -= thrd_RWLock__reader;
    if ((self->_impl.state & thrd_RWLock__reader_mask) == 0) {
        thrd_Cond_broadcast(&self->_impl.cond);
    }
    thrd_Mtx_unlock(&self->_impl.mtx);
};
