...
fn_((thrd_Group_spawn(thrd_Group* self, mem_Alctr gpa, Clsr$Void* clsr))(void) $scope) {
    ...
    let node = u_castP$((thrd_Group_Node*)(catch_((mem_Alctr_create($trace gpa, typeInfo$(thrd_Group_Node)))($ignore, {
    ...
    *node = (thrd_Group_Node){
    ...
    let instance = catch_((thrd_Group__spawnInst(self, gpa, clsr))($ignore, {
...
fn_((thrd_Group_adopt(thrd_Group* self, thrd_Group_Node* node, thrd_Self thrd))(void)) {
    node->self = thrd;
...
fn_((thrd_Group_wait(thrd_Group* self, thrd_wait_Src cancel_src))(Sched_Cancelable$void) $guard) {
    try_(thrd_Latch_wait(&self->latch, cancel_src));
    thrd_Group__joinThreads(self);
    return_ok({});
} $unguarded(fn);
fn_((thrd_Group_waitFor(thrd_Group* self, thrd_wait_Src cancel_src, time_Dur timeout))(Sched_TimedE$void) $guard) {
