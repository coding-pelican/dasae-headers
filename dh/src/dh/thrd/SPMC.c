...
$static fn_((thrd_SPMC__grip(thrd_SPMC* self))(ArrQue_Grip));
$static fn_((thrd_SPMC__remaining(time_Awake clock, time_Awake_Inst started, time_Dur timeout))(time_Dur));
...
fn_((thrd_SPMC_fini(thrd_SPMC* self, TypeInfo type))(void) $guard) {
    thrd_Mtx_lock(&self->lock);
    self->closed = true;
    var grip = thrd_SPMC__grip(self);
...
fn_((thrd_SPMC_tx(thrd_SPMC* self, TypeInfo type))(thrd_SPMC_Tx)) {
    return (thrd_SPMC_Tx){ .chan = self, .type = $typing(type) };
};

fn_((thrd_SPMC_Tx_send(thrd_SPMC_Tx self, u_V$raw item))(thrd_chan_WaitE$void) $guard) {
    ...
    var grip = thrd_SPMC__grip(self.chan);
...
fn_((thrd_SPMC_Tx_sendFor(thrd_SPMC_Tx self, u_V$raw item, time_Dur dur))(thrd_chan_TimedE$void) $guard) {
    ...
    while (!self.chan->closed && self.chan->len == self.chan->buf.len) {
        let remaining = thrd_SPMC__remaining(clock, started, dur);
...
fn_((thrd_SPMC_rx(thrd_SPMC* self, TypeInfo type))(thrd_SPMC_Rx)) {
    return (thrd_SPMC_Rx){ .chan = self, .type = $typing(type) };
};
...
fn_((thrd_SPMC_Rx_recv(thrd_SPMC_Rx self, u_V$raw out))(thrd_chan_WaitE$u_V$raw) $guard) {
    thrd_Mtx_lock(&self.chan->lock);
    defer_(thrd_Mtx_unlock(&self.chan->lock));
...
fn_((thrd_SPMC_Rx_recvFor(thrd_SPMC_Rx self, time_Dur dur, u_V$raw out))(thrd_chan_TimedE$u_V$raw) $guard) {
    let clock = catch_((time_Awake_direct())($ignore, time_Awake_noop));
    let started = time_Awake_now(clock);
...
fn_((thrd_SPMC_open(thrd_SPMC* self, TypeInfo type))(thrd_SPMC_Chans)) {
    return (thrd_SPMC_Chans)tie_(
        (thrd_SPMC_tx(self, type)),
        (thrd_SPMC_rx(self, type))
    );
};

fn_((thrd_SPMC_close(thrd_SPMC* self, TypeInfo type))(void) $guard) {
    let_ignore = type;
    thrd_Mtx_lock(&self->lock);
...
fn_((thrd_SPMC__grip(thrd_SPMC* self))(ArrQue_Grip)) {
    return ArrQue_grip((u_S$raw){ .raw = self->buf, .type = $typed(self->type) }, &self->head, &self->len);
};

fn_((thrd_SPMC__remaining(time_Awake clock, time_Awake_Inst started, time_Dur timeout))(time_Dur)) {
