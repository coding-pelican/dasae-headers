...
fn_((utf16_codeunitSeqLen(u16 first_codeunit))(utf16_E$utf16_SeqLen) $scope) {
    if (utf16_isHighSurrogate(first_codeunit)) return_ok(utf16_SeqLen_2);
    if (utf16_isLowSurrogate(first_codeunit)) return_err(E_cause$utf16_InvalidStartCodeunit());
...
fn_((utf16_codepointSeqLen(u32 codepoint))(utf16_E$utf8_SeqLen) $scope) {
    let handleUTF8 = utf16_codepointSeqLen__handleUTF8;
    return_ok(try_(handleUTF8(utf8_codepointSeqLen(codepoint))));
} $unscoped(fn);

fn_((utf16_encode(u32 codepoint, S$u16 out))(utf16_encode_E$S$u16) $scope) {
...
fn_((utf16_encodeWithin(u32 codepoint, S$u16 out))(utf16_E$S$u16) $scope) {
    if (utf16_isSurrogate(codepoint)) return_err(E_cause$utf16_TooLargeCodepoint());
...
fn_((utf16_decode(S_const$u16 codeunits))(utf16_E$u32) $scope) {
    if (codeunits.len == 0) return_err(E_cause$utf16_InvalidStartCodeunit());
    let first = *S_at((codeunits)[0]);
    if (utf16_isHighSurrogate(first)) {
        if (codeunits.len < 2) return_err(E_cause$utf16_DanglingSurrogateHalf());
        let second = *S_at((codeunits)[1]);
        return_(utf16_decodeSurrogatePair(first, second));
    } else if (utf16_isLowSurrogate(first)) {
        return_err(E_cause$utf16_UnexpectedSecondSurrogateHalf());
    } else {
        return_ok(as$(u32)(first));
    }
} $unscoped(fn);

fn_((utf16_decodeSurrogatePair(u16 high_codeunit, u16 low_codeunit))(utf16_E$u32) $scope) {
    if (!utf16_isHighSurrogate(high_codeunit)) return_err(E_cause$utf16_InvalidStartCodeunit());
    if (!utf16_isLowSurrogate(low_codeunit)) return_err(E_cause$utf16_ExpectedSecondSurrogateHalf());
    let high_half = as$(u32)(high_codeunit & int_maskLo_static$((u16)(10)));
    let low_half = as$(u32)(low_codeunit & int_maskLo_static$((u16)(10)));
...
fn_((utf16_Iter_next(utf16_Iter* self))(utf16_E$O$u32) $scope) {
    if (self->idx >= self->bytes.len) return_ok(none());
    let first = *S_at((self->bytes)[self->idx]);
    if (utf16_isHighSurrogate(first)) {
        if (self->idx + 1 >= self->bytes.len) return_err(E_cause$utf16_DanglingSurrogateHalf());
