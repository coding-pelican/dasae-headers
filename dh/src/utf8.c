...
$attr($must_check)
$static fn_((utf8__encode(u32 codepoint, utf8_SeqLen requested_len, S$u8 out))(utf8_E$S$u8));
$attr($must_check)
$static fn_((utf8__decodeSeqLen(S_const$u8 bytes, utf8_SeqLen seq_len))(utf8_E$u32));
...
fn_((utf8_codepointSeqLen(u32 codepoint))(utf8_E$utf8_SeqLen) $scope) {
    ...
    return_err(utf8_E_TooLargeCodepoint());
} $unscoped(fn);

fn_((utf8_byteSeqLen(u8 first_byte))(utf8_E$utf8_SeqLen) $scope) {
    if ((first_byte & int_maskHi_static$((u8)(1))) == 0) return_ok(utf8_SeqLen_1);
    ...
    return_err(utf8_E_InvalidStartByte());
} $unscoped(fn);
...
fn_((utf8_encodeWithin(u32 codepoint, S$u8 out))(utf8_E$S$u8) $scope) {
    return_(utf8__encode(codepoint, try_(utf8_codepointSeqLen(codepoint)), out));
} $unscoped(fn);

fn_((utf8__encode(u32 codepoint, utf8_SeqLen requested_len, S$u8 out))(utf8_E$S$u8) $scope) {
    ...
    case utf8_SeqLen_2:
        ...
        *S_at((out)[1]) = utf8__makeContinuationByte(intCast$((u8)(codepoint & int_maskLo_static$((u8)(6)))));
        break;
    case utf8_SeqLen_3:
        if (utf16_isSurrogate(codepoint)) return_err(utf8_E_EncodesSurrogateHalf());
    ...
    case utf8_SeqLen_4:
        ...
        *S_at((out)[1]) = utf8__makeContinuationByte(intCast$((u8)((codepoint >> 12) & int_maskLo_static$((u8)(6)))));
        *S_at((out)[2]) = utf8__makeContinuationByte(intCast$((u8)((codepoint >> 6) & int_maskLo_static$((u8)(6)))));
        *S_at((out)[3]) = utf8__makeContinuationByte(intCast$((u8)(codepoint & int_maskLo_static$((u8)(6)))));
        break;
    }
...
fn_((utf8_decode(S_const$u8 bytes))(utf8_E$u32) $scope) {
    if (bytes.len == 0) return_err(utf8_E_InvalidBytes());
    let seq_len = try_(utf8_byteSeqLen(*S_at((bytes)[0])));
    if (seq_len > bytes.len) return_err(utf8_E_InvalidBytes());
    return_(utf8__decodeSeqLen(bytes, seq_len));
} $unscoped(fn);

fn_((utf8__decodeSeqLen(S_const$u8 bytes, utf8_SeqLen seq_len))(utf8_E$u32) $scope) {
...
fn_((utf8_decode2(utf8_Decode2Buf bytes))(utf8_E$u32) $scope) {
    ...
    if (!utf8__isContinuation(*A_at((bytes)[1]))) return_err(utf8_E_ExpectedContinuation());
    val = (val << 6) | utf8__continuationPayload(*A_at((bytes)[1]));
    if (val < 0x80) return_err(utf8_E_OverlongEncoding());
    return_ok(val);
} $unscoped(fn);
...
fn_((utf8_decode3(utf8_Decode3Buf bytes))(utf8_E$u32) $scope) {
    ...
    if (!utf8__isContinuation(*A_at((bytes)[1]))) return_err(utf8_E_ExpectedContinuation());
    val = (val << 6) | utf8__continuationPayload(*A_at((bytes)[1]));
    if (!utf8__isContinuation(*A_at((bytes)[2]))) return_err(utf8_E_ExpectedContinuation());
    val = (val << 6) | utf8__continuationPayload(*A_at((bytes)[2]));
    if (val < 0x800) return_err(utf8_E_OverlongEncoding());
    if (utf16_isSurrogate(val)) return_err(utf8_E_EncodesSurrogateHalf());
    return_ok(val);
} $unscoped(fn);
...
fn_((utf8_decode4(utf8_Decode4Buf bytes))(utf8_E$u32) $scope) {
    ...
    if (!utf8__isContinuation(*A_at((bytes)[2]))) return_err(utf8_E_ExpectedContinuation());
    val = (val << 6) | utf8__continuationPayload(*A_at((bytes)[2]));
    if (!utf8__isContinuation(*A_at((bytes)[3]))) return_err(utf8_E_ExpectedContinuation());
    val = (val << 6) | utf8__continuationPayload(*A_at((bytes)[3]));
    if (val < 0x10000) return_err(utf8_E_OverlongEncoding());
    if (val > 0x10FFFF) return_err(utf8_E_TooLargeCodepoint());
    return_ok(val);
} $unscoped(fn);
...
fn_((utf8_view(S_const$u8 bytes))(utf8_E$utf8_View) $scope) {
    if (!utf8_validate(bytes)) return_err(utf8_E_InvalidBytes());
