...
$static fn_((HashSet__alloc(HashSet* self, TypeInfo key_ty, mem_Alctr gpa, u32 new_cap))(mem_E$void) $scope) {
    ...
    let max_align = pri_max(header_align, key_align);
...
$static fn_((HashSet__free(HashSet* self, TypeInfo key_ty, mem_Alctr gpa))(void)) {
...
$static fn_((HashSet__idx(HashSet self, u_V$raw key))(O$usize) $scope) {
    ...
    if (HashSet_cap(self) >= HashSet__simd_group_size && (HashSet_cap(self) % HashSet__simd_group_size) == 0) {
...
fn_((HashSet__idx_simd(HashSet self, u_V$raw key))(O$usize) $scope) {
    if (self.size == 0) { return_none(); }
    ...
    let start_group = start_idx / HashSet__simd_group_size;
    let start_offset_in_group = start_idx % HashSet__simd_group_size;
    let num_groups = cap / HashSet__simd_group_size;

    for_(($r(0, num_groups))(group_offset)) {
        let group_idx = (start_group + group_offset) % num_groups;
        let group_start = group_idx * HashSet__simd_group_size;
...
$static fn_((HashSet__grow(HashSet* self, TypeInfo key_ty, mem_Alctr gpa, u32 new_capacity))(mem_E$void) $scope) {
    let new_cap = pri_max(new_capacity, HashSet_default_min_cap);
...
fn_((HashSet_init(TypeInfo key_ty, P_const$HashSet_Ctx ctx, mem_Alctr gpa, u32 cap))(mem_E$HashSet) $scope) {
    claim_assert_nonnull(ctx);
...
fn_((HashSet_clone(HashSet self, TypeInfo key_ty, mem_Alctr gpa))(mem_E$HashSet) $scope) {
...
fn_((HashSet_rehash(HashSet* self, TypeInfo key_ty))(void)) {
    ...
    var_(curr, u32) = 0;
    while (curr < cap) {
        let curr_ctrl = HashSet__metadataAt(*self, curr);
        ...
        while ((idx < curr && HashMap_Ctrl_isUsed(*HashSet__metadataAt(*self, idx)))
               || (idx > curr && HashSet__metadataAt(*self, idx)->fingerprint == HashMap_Ctrl_tombstone)) {
            idx = (idx + 1) & mask;
        }
        if (idx < curr) {
            ...
            if (was_used) {
                ...
                curr_ctrl->used = 0;
                u_memset0(HashSet__keyAt(*self, key_ty, curr));
                curr++;
            }
        }
    }
