...
fn_((Thrd_ResetEvent_init(void))(Thrd_ResetEvent)) {
    return (Thrd_ResetEvent){
        .state = atom_V_init(Thrd_ResetEvent_unset),
    };
};

fn_((Thrd_ResetEvent_fini(Thrd_ResetEvent* self))(void)) {
    atom_V_store(&self->state, Thrd_ResetEvent_unset, atom_MemOrd_monotonic);
};
...
fn_((Thrd_ResetEvent_set(Thrd_ResetEvent* self))(void)) {
    if (atom_V_load(&self->state, atom_MemOrd_monotonic) == Thrd_ResetEvent_is_set) {
        return;
    }
    if (atom_V_fetchXchg(&self->state, Thrd_ResetEvent_is_set, atom_MemOrd_release) == Thrd_ResetEvent_waiting) {
...
fn_((Thrd_ResetEvent_isSet(const Thrd_ResetEvent* self))(bool)) {
    return atom_V_load(&self->state, atom_MemOrd_acquire) == Thrd_ResetEvent_is_set;
};
...
fn_((Thrd_ResetEvent__waitUntilSet(Thrd_ResetEvent* self, O$time_Duration timeout))(Thrd_ResetEvent_Err$void) $scope) {
    ...
    if (state == Thrd_ResetEvent_unset) {
        state = orelse_((atom_V_cmpXchgStrong$(
            O$u32, &self->state, state, Thrd_ResetEvent_waiting, atom_MemOrd_acquire, atom_MemOrd_acquire
        ))(Thrd_ResetEvent_waiting));
    }
    if (state == Thrd_ResetEvent_waiting) {
        var futex_deadline = Thrd_Ftx_Deadline_init(timeout);
        while (true) {
            let waiting = Thrd_Ftx_Deadline_wait(&futex_deadline, &self->state, Thrd_ResetEvent_waiting);
            state = atom_V_load(&self->state, atom_MemOrd_acquire);
            if (state != Thrd_ResetEvent_waiting) { break; }
            try_(waiting);
        }
    }
    claim_assert(state == Thrd_ResetEvent_is_set);
    return_ok({});
} $unscoped_(fn);
