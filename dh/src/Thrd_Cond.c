...
$attr($inline_always)
$static fn_((Thrd_Cond__init(void))(Thrd_Cond));
$attr($inline_always)
$static fn_((Thrd_Cond__fini(Thrd_Cond* self))(void));
$attr($must_check)
$static fn_((Thrd_Cond__wait(Thrd_Cond* self, Thrd_Mtx* mtx, O$time_Duration timeout))(Thrd_Cond_Err$void));
$static fn_((Thrd_Cond__wake(Thrd_Cond* self, Thrd_Cond__Notify notify))(void));
...
fn_((Thrd_Cond_wait(Thrd_Cond* self, Thrd_Mtx* mtx))(void) $scope) {
    return_void(catch_((Thrd_Cond__wait(self, mtx, none$((O$time_Duration))))($ignore, claim_unreachable)));
} $unscoped_(fn);

fn_((Thrd_Cond_timedWait(Thrd_Cond* self, Thrd_Mtx* mtx, time_Duration timeout))(Thrd_Cond_Err$void)) {
    return Thrd_Cond__wait(self, mtx, some$((O$time_Duration)(timeout)));
};

fn_((Thrd_Cond_signal(Thrd_Cond* self))(void)) {
    Thrd_Cond__wake(self, Thrd_Cond__Notify_one);
};

fn_((Thrd_Cond_broadcast(Thrd_Cond* self))(void)) {
    Thrd_Cond__wake(self, Thrd_Cond__Notify_all);
};

#if plat_type == plat_type_windows
$attr($inline_always)
$static fn_((Thrd_Cond__init(void))(Thrd_Cond)) {
    return (Thrd_Cond){ .impl = CONDITION_VARIABLE_INIT };
};
...
fn_((Thrd_Cond__wait(Thrd_Cond* self, Thrd_Mtx* mtx, O$time_Duration timeout))(Thrd_Cond_Err$void) $scope) {
...
fn_((Thrd_Cond__wake(Thrd_Cond* self, Thrd_Cond__Notify notify))(void)) {
    switch (notify) {
    case_((Thrd_Cond__Notify_one)) WakeConditionVariable(&self->impl); break $end(case);
    case_((Thrd_Cond__Notify_all)) WakeAllConditionVariable(&self->impl); break $end(case);
    }
};

#elif plat_type == plat_type_linux || plat_type == plat_type_darwin
#define Thrd_Cond__one_waiter 1
#define Thrd_Cond__waiter_mask 0xffff

#define Thrd_Cond__one_signal 1 << 16
#define Thrd_Cond__signal_mask 0xffff << 16
...
$attr($inline_always)
$static fn_((Thrd_Cond__fini(Thrd_Cond* self))(void)) {
    atom_V_store(&self->epoch, 0, atom_MemOrd_monotonic);
...
fn_((Thrd_Cond__wait(Thrd_Cond* self, Thrd_Mtx* mtx, O$time_Duration timeout))(Thrd_Cond_Err$void) $guard) {
    var epoch = atom_V_load(&self->epoch, atom_MemOrd_acquire);
    var state = atom_V_fetchAdd(&self->state, Thrd_Cond__one_waiter, atom_MemOrd_monotonic);
    debug_assert(state & Thrd_Cond__waiter_mask != Thrd_Cond__waiter_mask);
    state += Thrd_Cond__one_waiter;
    ...
    while (true) {
        catch_((Thrd_Ftx_Deadline_wait(&deadline, &self->epoch, epoch))(err, {
            ...
            while (true) {
                ...
                while (state & Thrd_Cond__signal_mask != 0) {
                    let new_state = state - Thrd_Cond__one_waiter - Thrd_Cond__one_signal;
                    state = orelse_((atom_V_cmpXchgWeak(&self->state, state, new_state, atom_MemOrd_acquire, atom_MemOrd_monotonic))(return_ok({})));
                }

                // Remove the waiter we added and officially return timed out.
                let new_state = state - Thrd_Cond__one_waiter;
                state = orelse_((atom_V_cmpXchgWeak(&self->state, state, new_state, atom_MemOrd_monotonic, atom_MemOrd_monotonic))(return_err(err)));
            }
        }));

        epoch = atom_V_load(&self->epoch, atom_MemOrd_acquire);
        state = atom_V_load(&self->state, atom_MemOrd_monotonic);
...
fn_((Thrd_Cond__wake(Thrd_Cond* self, Thrd_Cond__Notify notify))(void)) {
    var state = atom_V_load(&self->state, atom_MemOrd_monotonic);
    while (true) {
        let waiters = (state & Thrd_Cond__waiter_mask) / Thrd_Cond__one_waiter;
        let signals = (state & Thrd_Cond__signal_mask) / Thrd_Cond__one_signal;
        ...
        let new_state = state + (Thrd_Cond__one_signal * to_wake);
        state = orelse_((atom_V_cmpXchgWeak(&self->state, state, new_state, atom_MemOrd_release, atom_MemOrd_monotonic))({
            ...
            let_ignore = atom_V_fetchAdd(&self->epoch, 1, atom_MemOrd_release);
            return Thrd_Ftx_wake(&self->epoch, to_wake);
        }));
    }
};
#endif
