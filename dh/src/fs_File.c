...
$static
fn_((windows_CloseHandle(HANDLE handle))(void)) {
    CloseHandle(handle);
}
$static
fn_((windows_WriteFile(HANDLE handle, Sli_const$u8 bytes))(Err$usize) $scope) {
...
#include <unistd.h>
$static
fn_((linux_close(posix_fd_t handle))(void)) {
    close(handle);
}
$static
fn_((linux_write(posix_fd_t handle, Sli_const$u8 bytes))(Err$usize) $scope) {
...
$static
fn_((Writer_VT_write(const anyptr ctx, Sli_const$u8 bytes))(Err$usize)) {
...
$static
fn_((Writer_init(fs_File file))(Writer)) {
