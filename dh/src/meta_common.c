...
P_const$raw Sli_rawAt(TypeInfo type, P_const$raw ptr, usize len, usize index) {
    ...
    debug_assert_fmt(index < len, "Index out of bounds (index: {:uz}, len: {:uz})", index, len);
...
P_const$raw Sli_rawSlice(TypeInfo type, P_const$raw ptr, usize len, usize begin, usize end) {
    $unused(len, end);
    debug_assert_nonnull(ptr);
    debug_assert_fmt(begin <= end, "Invalid range (begin: {:uz}, end: {:uz})", begin, end);
    debug_assert_fmt(end <= len, "Slice out of bounds (end: {:uz}, len: {:uz})", end, len);
...
fn_((meta_S_at(meta_S$raw self, usize index))(meta_P$raw)) {
    debug_assert_nonnull(self.addr);
    debug_assert_fmt(index < self.len, "Index out of bounds (index: {:uz}, len: {:uz})", index, self.len);
    return (meta_P$raw){
        .type = self.type,
...
fn_((meta_S_slice(meta_S$raw self, usize begin, usize end))(meta_S$raw)) {
    debug_assert_nonnull(self.addr);
    debug_assert_fmt(begin < end, "Invalid range (begin: {:uz}, end: {:uz})", begin, end);
    debug_assert_fmt(end <= self.len, "Slice out of bounds (end: {:uz}, len: {:uz})", end, self.len);
    return (meta_S$raw){
        .type = self.type,
