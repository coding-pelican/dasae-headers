...
fn_((utf16_codeunitSeqLen(u16 first_codeunit))(utf16_E$utf16_SeqLen) $scope) {
    ...
    if (utf16_isLowSurrogate(first_codeunit)) return_err(E_cause$UTF16InvalidStartCodeUnit());
...
fn_((utf16_encode(u32 codepoint, S$u16 out))(E$S$u16) $scope) {
    if (utf16_isSurrogate(codepoint)) return_err(E_cause$UTF16CodepointTooLarge());
    let required_len = (codepoint < 0x10000) ? as$(usize)(1) : as$(usize)(2);
    if (codepoint > 0x10FFFF) return_err(E_cause$UTF16CodepointTooLarge());
...
fn_((utf16_decode(S_const$u16 codeunits))(utf16_E$u32) $scope) {
    if (codeunits.len == 0) return_err(E_cause$UTF16InvalidStartCodeUnit());
    let first = *S_at((codeunits)[0]);
    if (utf16_isHighSurrogate(first)) {
        if (codeunits.len < 2) return_err(E_cause$UTF16DanglingSurrogateHalf());
        let second = *S_at((codeunits)[1]);
        return_(utf16_decodeSurrogatePair(first, second));
    } else if (utf16_isLowSurrogate(first)) {
        return_err(E_cause$UTF16UnexpectedSecondSurrogateHalf());
    } else {
...
fn_((utf16_decodeSurrogatePair(u16 high_codeunit, u16 low_codeunit))(utf16_E$u32) $scope) {
    if (!utf16_isHighSurrogate(high_codeunit)) return_err(E_cause$UTF16InvalidStartCodeUnit());
    if (!utf16_isLowSurrogate(low_codeunit)) return_err(E_cause$UTF16ExpectedSecondSurrogateHalf());
...
fn_((utf16_Iter_next(utf16_Iter* self))(utf16_E$O$u32) $scope) {
    ...
    if (utf16_isHighSurrogate(first)) {
        if (self->idx + 1 >= self->bytes.len) return_err(E_cause$UTF16DanglingSurrogateHalf());
