...
$static fn_((mp_ThrdPool_printStats(mp_ThrdPool* pool))(void)) {
    ...
    io_stream_println(u8_l("Avg Tasks per Worker: {:.1fl}"), as$(f32)(total_tasks) / as$(f32)(pool->threads.len));
};
    ...
    while (atom_V_load(&pool->running, atom_MemOrd_acquire)) {
        ...
        using_fini_(Thrd_Mtx_lock(&pool->mutex), Thrd_Mtx_unlock(&pool->mutex)) {
            ...
            if (atom_V_load(&pool->count, atom_MemOrd_monotonic) > 0) {
                ...
                asg_l((&maybe_task)(some(S_at((pool->tasks)[tail % mp_max_task_count]))));
...
$attr($must_check)
$static fn_((mp_ThrdPool_init(mem_Alctr gpa, usize thrd_count))(E$P$mp_ThrdPool) $guard) {
    let_(pool, mp_ThrdPool*) = u_castP$((mp_ThrdPool*)(try_((mem_Alctr_create(gpa, typeInfo$(InnerType))))));
    let field_types = typeInfos$(S_InnerT$(FieldType$(mp_ThrdPool, workers)), S_InnerT$(FieldType$(mp_ThrdPool, threads)));
    let field_mem = try_(mem_Alctr_create(gpa, u_typeInfoRecordN(thrd_count, field_types)));
    errdefer_($ignore, mem_Alctr_destroy(gpa, field_mem));
    ...
    asg_l((pool)($init((LitType){
        $field((workers)),
        $field((threads)),
        $field((tasks)$asg(u_castS$((FieldType)(try_(mem_Alctr_alloc(
...
$static fn_((mp_ThrdPool_fini(mp_ThrdPool** self, mem_Alctr gpa))(void)) {
    mp_ThrdPool_shutdown(*self);
    mem_Alctr_free(gpa, u_anyS((*self)->tasks));
    let field_types = typeInfos$(S_InnerT$(FieldType$(mp_ThrdPool, workers)), S_InnerT$(FieldType$(mp_ThrdPool, threads)));
    let record_mem = u_recordNPtrMut(u_anyS((*self)->workers), (*self)->workers.len, field_types, 0);
    mem_Alctr_destroy(gpa, record_mem);
    mem_Alctr_destroy(gpa, u_anyP(*self));
    *self = null;
};
...
$static fn_((mp_parallel_for_pooled(mp_ThrdPool* pool, R range, mp_ThrdPool_TaskFn fn, u_V$raw params))(void)) {
    ...
    for_(($r(0, pool->threads.len))(t) {
        ...
        if (begin < end) {
            asg_l((A_at((task_buf)[task_count++]))({
                .range = $r(begin, end),
                .workerFn = fn,
                .params = params,
            }));
...
$static fn_((RandGaussian_init(Rand rng))(RandGaussian)) {
    return l$((RandGaussian){
        .rng $like_deref = rng,
        .spare = none(),
    });
};
$static fn_((RandGaussian_next$f32(RandGaussian* self, f32 mean, f32 std_dev))(f32)) {
    ...
    f32 s = f32_nan;
    do {
        u = (Rand_next$f32(self->rng) * 2.0 - 1.0);
        v = (Rand_next$f32(self->rng) * 2.0 - 1.0);
        s = u * u + v * v;
    } while (s >= 1.0 || s == 0.0);
    s = flt_sqrt(-2.0 * flt_ln(s) / s);
    asg_l((&self->spare)(some(v * s)));
...
$static fn_((State_initParticles(mp_ThrdPool* pool, S$Particle particles, m_V2f32 center, m_V2f32 radius_a_b))(void));
$static fn_((State_init(mp_ThrdPool* pool, S$Particle particles, S$Cell grid))(State)) {
    State_initParticles(pool, particles, m_V2f32_zero, m_V2f32_from(State_boundary_radius, 200.0));
    return l$((State){
        .pool = pool,
        .particles = particles,
        .grid = grid,
...
$static fn_((State_simulateFrame_phased(State* self, mp_ThrdPool* pool))(void)) {
    ...
    for_(($r(0, thrd_count))(t) {
        ...
        let end = pri_min(begin + grid_chunk, State_grid_width * State_grid_height);
        if (begin >= end) { continue; }
        asg_l((A_at((phased_args_clearGrid)[t]))({
    ...
    for_(($r(0, thrd_count))(t) {
        ...
        let end = pri_min(begin + particles_chunk, State_particles);
        if (begin >= end) { continue; }
        asg_l((A_at((phased_args_buildGrid)[t]))({
    ...
    for_(($r(0, thrd_count))(t) {
        ...
        let end = pri_min(begin + particles_chunk, State_particles);
        if (begin >= end) { continue; }
        asg_l((A_at((phased_args_gravity)[t]))({
    ...
    for_(($r(0, thrd_count))(t) {
        ...
        let end = pri_min(begin + particles_chunk, State_particles);
        if (begin >= end) { continue; }
        asg_l((A_at((phased_args_collisions)[t]))({
...
fn_((main(S$S_const$u8 args))(E$void) $guard) {
    ...
    var gpa = heap_Page_alctr(&(heap_Page){});
    ...
    io_stream_println(u8_l("\nAllocating memory..."));
    let particles = try_(u_castE$((E$$(S$Particle))(mem_Alctr_alloc(gpa, typeInfo$(Particle), State_particles))));
    defer_(mem_Alctr_free(gpa, u_anyS(particles)));
    let grid = try_(u_castE$((E$$(S$Cell))(mem_Alctr_alloc(gpa, typeInfo$(Cell), State_grid_width * State_grid_height))));
    defer_(mem_Alctr_free(gpa, u_anyS(grid)));
    io_stream_println(u8_l("Memory allocated: {:.2fl} MB"), as$(f32)(State_particles * typeInfo$(Particle).size) / (1024.0 * 1024.0));
...
fn_((State_initParticles_worker(R range, u_V$raw params))(void)) {
    ...
    for_(($s(targets))(particle) {
        var self_i = *particle;
        var r = RandGaussian_next$f32(State_rngGaussian(), 0.0, r_b / 2.5);
        if (r > r_b) { r = r_b; }
        if (r < 0) { r = -r; }
        let theta = m_V2f32_sincos(Rand_next$f32(State_rng()) * math_f32_tau);
        var p = m_V2f32_mul(m_V2f32_addScalar(c, r), theta);
        let d = m_V2f32_len(p);
        if (d > r_a) {
            m_V2f32_scaleAsg(&p, r_a / d);
        }
        self_i.pos = p;
        self_i.vel = m_V2f32_zero;
        self_i.mass = 1.0;
        *particle = self_i;
    });
};

fn_((State_initParticles(mp_ThrdPool* pool, S$Particle particles, m_V2f32 center, m_V2f32 radius_a_b))(void)) {
    mp_parallel_for_pooled(
        pool, $r(0, State_particles), State_initParticles_worker,
        u_anyV(l$((State_InitParticlesArgs){
            .particles = particles,
            .center = center,
...
fn_((State_hashPosition(m_V2f32 pos))(usize)) {
    ...
    let gx_1 = as$(usize)(pri_clamp(gx_0, 0, as$(isize)(State_grid_width)-1));
    let gy_1 = as$(usize)(pri_clamp(gy_0, 0, as$(isize)(State_grid_height)-1));
...
fn_((State_buildSpatialGrid_worker(R range, u_V$raw params))(void)) {
    ...
    for_(($r(0, State_grid_width * State_grid_height))(cell_idx) {
        ...
        let copy_count = pri_min(local_cell->count, State_max_particles_per_cell - global_pos);
...
fn_((State_applyGravityAndUpdate(State* self))(void)) {
    mp_parallel_for_pooled(
        ...
        u_anyV(l$((State_ApplyGravityAndUpdateArgs){
            .particles = self->particles,
        }))
    );
};
    ...
    for_(((range), $s(targets))(i, particle) {
        ...
        for_(($r(0, 3))(dy_offset) {
            for_(($r(0, 3))(dx_offset) {
                ...
                let limit = pri_min(count, State_max_particles_per_cell);

                // 셀 중심까지의 거리로 빠른 컬링
                let cell_center_x = (as$(f32)(nx)-100.0) * State_cell_size + State_cell_size * 0.5;
                let cell_center_y = (as$(f32)(ny)-100.0) * State_cell_size + State_cell_size * 0.5;
...
fn_((State_handleCollisions(State* self))(void)) {
    mp_parallel_for_pooled(
        ...
        u_anyV(l$((State_HandleCollisionsArgs){
            .particles = self->particles,
            .grid = self->grid,
        }))
    );
