...
$static fn_((State_init(S$Particle particles, S$Cell grid))(State)) {
    ...
    *State_rngGaussian() = RandGaussian_init(*State_rng());
    State_initParticles(particles, m_V2f64_zero, m_V2f64_of(State_boundary_radius, 200.0));
    return l$((State){
        .particles = particles,
        .grid = grid,
    });
...
fn_((State_initParticles(S$Particle particles, m_V2f64 center, m_V2f64 radius_a_b))(void)) {
    mp_parallel_for(
        $r(0, State_particles), State_initParticles_worker,
...
fn_((State_clearGrid(S$Cell grid))(void)) {
    mp_parallel_for(
        $r(0, State_grid_width * State_grid_height), State_clearGrid_worker,
...
fn_((State_applyGravity_worker(usize i, u_V$raw params))(void)) {
    ...
    if (d > 0.1f) {
        ...
        m_V2f64_addAsg(&particle.vel, m_V2f64_scal(dp, (1.0 / d) * force * State_delta_time));
        // particle->vel.x += (dx / dist) * force * State_delta_time;
        // particle->vel.y += (dy / dist) * force * State_delta_time;
    }
    m_V2f64_scalAsg(&particle.vel, State_damping);
...
fn_((State_applyGravity(State* self))(void)) {
    mp_parallel_for(
        $r(0, State_particles), State_applyGravity_worker,
        u_anyV(l$((State_ApplyGravityArgs){
...
fn_((State_handleCollisions(State* self))(void)) {
    mp_parallel_for(
        $r(0, State_particles), State_handleCollisions_worker,
...
fn_((State_updatePositions_worker(usize i, u_V$raw params))(void)) {
    ...
    m_V2f64_addAsg(&particle.pos, m_V2f64_scal(particle.vel, State_delta_time));
    let d = m_V2f64_len(particle.pos);
    if (d > State_boundary_radius) {
        let n = m_V2f64_scalInv(particle.pos, d);
        particle.pos = m_V2f64_scal(n, State_boundary_radius);
        let dot = m_V2f64_dot(particle.vel, n);
        m_V2f64_subAsg(&particle.vel, m_V2f64_scal(n, dot * 2.0f));
        m_V2f64_scalAsg(&particle.vel, 0.8);
    }
...
fn_((State_updatePositions(State* self))(void)) {
    mp_parallel_for(
        $r(0, State_particles), State_updatePositions_worker,
        u_anyV(l$((State_UpdatePositionsArgs){
            .particles = self->particles,
        }))
    );
}
